...
import { 
    ...
    Check, X, GripVertical, ChevronUp, ChevronDown, CircleDashed, User, Target, AlertCircle
} from 'lucide-react';
import * as LucideIcons from 'lucide-react';
import { saveSettings, bulkUpdateProjects, bulkUpdateClients, bulkUpdateServices, addGlobalLog } from '../api/dbService';
import { toast } from '../utils/toast';

const ICONS = ["Activity", "AlarmClock", "AlertCircle", "AlertTriangle", "Archive", "Award", "Ban", "BarChart", "Bell", "Blocks", "BookOpen", "Bookmark", "Briefcase", "Building", "Building2", "Calendar", "CalendarCheck", "CalendarClock", "CalendarX", "Camera", "CheckCircle2", "CheckSquare", "CircleCheckBig", "CircleDashed", "CircleEllipsis", "CircleX", "ClipboardCheck", "ClockAlert", "Cloud", "Contact", "Download", "FileText", "Flag", "Hammer", "Heart", "House", "HousePlus", "Inbox", "Key", "Layers", "Loader", "Loader2", "Mail", "Map", "Monitor", "Package", "PackageCheck", "Pause", "PauseCircle", "PieChart", "Play", "PlayCircle", "PlusCircle", "Rocket", "Search", "Send", "Settings", "Shield", "ShieldCheck", "Smartphone", "Star", "StopCircle", "Target", "ThumbsUp", "TrendingUp", "Trophy", "User", "Users", "Wrench", "Zap"];
const COLORS = [
    "red", "crimson", "rose", "pink", "fuchsia",
    "purple", "violet", "indigo", "blue", "ocean",
    "sky", "cyan", "teal", "emerald", "green",
    "forest", "lime", "yellow", "gold", "amber",
    "orange", "stone", "slate", "slateDark", "navy"
];
...
const COLOR_MAP: Record<string, string> = {
    red: '#ef4444', crimson: '#be123c', rose: '#f43f5e', pink: '#ec4899', fuchsia: '#d946ef',
    purple: '#a855f7', violet: '#8b5cf6', indigo: '#6366f1', blue: '#3b82f6', ocean: '#0369a1',
    sky: '#0ea5e9', cyan: '#06b6d4', teal: '#14b8a6', emerald: '#10b981', green: '#22c55e',
    forest: '#15803d', lime: '#84cc16', yellow: '#eab308', gold: '#d4af37', amber: '#f59e0b',
    orange: '#f97316', stone: '#78716c', slate: '#64748b', slateDark: '#334155', navy: '#1e3a8a'
};
...
function CustomPicker({ 
    ...
    return (
        <div className="relative shrink-0" ref={ref}>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-1 bg-white border border-border rounded-lg shadow-xl p-2 z-50 animate-in fade-in zoom-in-95 duration-100" style={{ width: type === 'icon' ? '430px' : '220px' }}>
                    {type === 'color' ? (
...
export default function SettingsDraft() {
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            if (oldName !== newName.trim()) {
                ...
                customMsg = logMsg;
                if (cascadedCount && cascadedCount > 0) customMsg += ` (Cascaded to ${cascadedCount} records)`;
            } else {
    ...
    const handleConfirmDelete = () => {
        ...
        if (field === 'serviceOutcomes' || field === 'serviceStatuses') {
            ...
            newSettings.archivedData.settingsData.push({ ...deletedItem, archivedAt: new Date().toISOString() });
        } else {
            const currentList = [...(settings[field as keyof typeof settings] as any[])];
        ...
        const deletedName = typeof deletedItem === 'string' ? deletedItem : deletedItem?.name || 'Unknown';
        const displayField = fieldDisplayNames[field] || field;
                                ...
                                return (
                                    <li key={i} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-slate-50 border-l-4 border-l-primary relative z-10 shadow-sm">
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            ...
                                            <button onClick={handleSaveEdit} className="bg-primary text-primary-foreground rounded-md text-xs font-medium h-8 px-3 shadow-sm hover:bg-primary/90 transition-all active:scale-95">Save</button>
                                        </div>
                                    </li>
                                );
                            }
                                ...
                                return (
                                    <li key={i} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-destructive/5 relative z-10">
                                        <div className="text-sm font-semibold text-destructive flex items-center gap-2">
                                            <AlertCircle className="w-4 h-4" /> Archive "{name}"? This removes it from active dropdowns, but preserves historical records.
                                        </div>
                                        <div className="flex items-center gap-2 shrink-0">
                                            ...
                                            <button onClick={handleConfirmDelete} className="bg-destructive text-destructive-foreground rounded-md text-xs font-medium h-8 px-3 shadow-sm hover:bg-destructive/90 transition-all active:scale-95">Archive</button>
                                        </div>
                                    </li>
                                );
                            }
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        {!isStringList && !isServices ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    {(fieldName === 'managers' || item.icon) && renderIcon(fieldName === 'managers' ? 'User' : item.icon, "w-3 h-3")}
                                                    {name}
                                                </span>
                                            </div>
                                        ) : (
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <div className="cursor-grab active:cursor-grabbing p-1.5 text-muted-foreground hover:text-primary hover:bg-slate-200 rounded-md transition-colors" title="Drag to reorder">
                                            <GripVertical className="w-4 h-4" />
                                        </div>
                                        <button onClick={() => moveItem(fieldName, i, -1, items)} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-slate-200 rounded-md transition-colors" title="Move Up">
                                            <ChevronUp className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => moveItem(fieldName, i, 1, items)} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-slate-200 rounded-md transition-colors" title="Move Down">
                                        ...
                                        }} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-slate-200 rounded-md transition-colors" title="Edit">
                                            <Edit2 className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => setDeletingItem({ field: fieldName, idx: i, globalIdx })} className="p-1.5 text-muted-foreground hover:text-destructive hover:bg-red-50 rounded-md transition-colors" title="Archive">
                ...
                <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-white custom-thin-scroll">
                    ...
                    {activeTab === 'workflow' && (
                        <div className="max-w-3xl animate-in fade-in duration-300">
                            ...
                            {renderList("Implementation Milestones", "Milestones for project execution.", 'phases')}
                    ...
                    {activeTab === 'scoring' && (
                        <div className="max-w-4xl animate-in fade-in duration-300 space-y-8">
                                ...
                                <div className="space-y-6">
                                    {[
                                        ...
                                        <div key={item.key} className="flex items-center gap-4">
                                            <label className="text-sm font-semibold text-slate-600 w-48 shrink-0">{item.label}</label>
                                            <input 
                                            ...
                                            />
                                            <span className="text-sm font-semibold text-slate-800 w-12 text-right">{item.val}%</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="mt-6 pt-6 border-t border-border flex justify-between items-center">
                                    <span className={`text-sm font-bold ${totalClientWeights === 100 ? 'text-lime-600' : 'text-red-500'}`}>
                            ...
                            <div className="bg-white border border-border rounded-xl shadow-sm p-6">
                                ...
                                <div className="mt-6 pt-6 border-t border-border flex justify-between items-center">
                                    <span className={`text-sm font-bold ${totalProjectWeights === 100 ? 'text-lime-600' : 'text-red-500'}`}>
                            ...
                            <div className="bg-white border border-border rounded-xl shadow-sm p-6">
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    <div className="flex flex-col gap-2">
                                        <label className="text-sm font-semibold text-slate-500">Healthy Threshold</label>
                                        <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-xs text-slate-500">Scores above this will display green.</span>
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <label className="text-sm font-semibold text-slate-500">At-Risk Threshold</label>
                                        <div className="flex items-center gap-3">
                                            <span className="text-sm font-medium text-slate-600">&lt;</span>
                                            <input 
                                                type="number" 
                                        ...
                                        <span className="text-xs text-slate-500">Scores below this will display red.</span>
                                    </div>
                                </div>
                            </div>
                        </div>
