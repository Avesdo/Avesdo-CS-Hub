...
import { getSystemLogs, restoreRecord, hardDeleteRecord, saveSettings, addAutoLog, addServiceAutoLog, addProjectAutoLog, getPendingAliases, resolveAlias, addGlobalLog } from '../api/dbService';
import { toast } from '../utils/toast';

export default function AdminHub() {
    const [activeTab, setActiveTab] = useState<'audit' | 'archives' | 'intake'>('audit');
    const { settings, archivedClients, archivedProjects, archivedServices, services, projects, clients, user } = useAppState();
    ...
    const handleHardDeleteRecordLocal = async (collectionName: string, item: any) => {
        ...
        toast.promise(hardDeleteRecord(collectionName, item.clientId || item.id, title, { silent: true }).then(async () => {
            ...
            await addGlobalLog(`Permanently deleted archived record`, entityType, item.clientId || item.id, title, author);
            loadLogs();
    ...
    const handleRestoreSetting = async (category: string, index: number, isSettingsData: boolean) => {
        ...
        const restoredName = typeof restoredItem === 'string' ? restoredItem : restoredItem?.name || 'Unknown';
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-4">
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs text-muted-foreground">
                                    <span className="font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md capitalize text-xs">{type}</span>
