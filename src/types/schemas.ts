...
export const ProjectSchema = z.object({
  ...
  healthScore: z.union([z.number(), z.literal('N/A')]).optional().catch('N/A'),
  trendData: z.array(z.any()).optional().catch([]),
  onboardingCsat: z.object({
...
export const SettingsSchema = z.object({
  managers: z.array(z.object({ name: z.string().catch(''), color: z.string().catch(''), icon: z.string().optional() })).catch([]),
  clientTypes: z.array(z.object({ name: z.string().catch(''), color: z.string().catch(''), icon: z.string().optional() })).catch([]),
  features: z.array(z.string()).catch([]),
  services: z.array(z.object({ name: z.string().catch(''), price: z.number().catch(0) })).catch([]),
  statuses: z.array(z.object({ name: z.string().catch(''), color: z.string().catch(''), icon: z.string().optional() })).catch([]),
  timelines: z.array(z.object({ name: z.string().catch(''), color: z.string().catch(''), icon: z.string().optional() })).catch([]),
  phases: z.array(z.object({ name: z.string().catch(''), color: z.string().catch(''), icon: z.string().optional() })).catch([]),
  scoring: z.object({
    weights: z.object({
  ...
  }).catch({ weights: { opActivity: 20, featAdoption: 20, userVol: 20, csat: 20, financial: 20 }, thresholds: { healthy: 70, warning: 40 } })
}).passthrough();
