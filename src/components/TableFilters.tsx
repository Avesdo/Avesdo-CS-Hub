...
export const DateFilter = ({ dateRange, setDateRange }: any) => {
    ...
    useEffect(() => {
        if (isOpen) {
            if (dateRange?.start) {
                ...
                setLocalStartYear(y); setLocalStartMonth(m);
            } else {
                setLocalStartYear(''); setLocalStartMonth('');
            }
            if (dateRange?.end) {
                const [y, m] = dateRange.end.split('-');
                setLocalEndYear(y); setLocalEndMonth(m);
            } else {
                setLocalEndYear(''); setLocalEndMonth('');
            }
        }
    }, [isOpen, dateRange]);
    ...
    const handleClear = () => {
        setLocalStartMonth(''); setLocalStartYear('');
        setLocalEndMonth(''); setLocalEndYear('');
        setDateRange(null);
        setIsOpen(false);
    };
    ...
    return (
        <div className="relative inline-block ml-1" ref={ref}>
            ...
            {isOpen && (
                <div className="absolute top-full right-0 mt-2 w-[440px] flex bg-white border border-border rounded-xl shadow-xl z-50 font-normal cursor-default" onClick={e => e.stopPropagation()}>
                    
                    <div className="w-[140px] bg-slate-50 border-r border-border p-2 flex flex-col gap-1 shrink-0 rounded-l-xl">
                        <div className="text-[11px] font-bold text-muted-foreground px-2 py-1 mb-1">Quick Select</div>
                        {[
                    ...
                    <div className="flex-1 p-4 flex flex-col">
                        ...
                        <div className="flex gap-3 mb-5 flex-1">
                            <div className="flex-1 flex flex-col gap-2">
                                ...
                                <Select 
                                    ...
                                    trigger={
                                        <button type="button" className="w-full flex items-center justify-between border border-input rounded-md px-3 py-2 text-sm bg-white hover:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all h-[38px]">
                                            <span className={localStartMonth ? 'text-foreground' : 'text-muted-foreground truncate'}>{localStartMonth ? months.find(m => m.value === localStartMonth)?.label : 'Select Month'}</span>
                                ...
                                <Select 
                                    ...
                                    trigger={
                                        <button type="button" className="w-full flex items-center justify-between border border-input rounded-md px-3 py-2 text-sm bg-white hover:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all h-[38px]">
                                            <span className={localStartYear ? 'text-foreground' : 'text-muted-foreground truncate'}>{localStartYear || 'Select Year'}</span>
                                            <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0 opacity-50" />
                                        </button>
                                    }
                                />
                            ...
                            <div className="flex-1 flex flex-col gap-2">
                                ...
                                <Select 
                                    ...
                                    trigger={
                                        <button type="button" className="w-full flex items-center justify-between border border-input rounded-md px-3 py-2 text-sm bg-white hover:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all h-[38px]">
                                            <span className={localEndMonth ? 'text-foreground' : 'text-muted-foreground truncate'}>{localEndMonth ? months.find(m => m.value === localEndMonth)?.label : 'Select Month'}</span>
                                            <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0 opacity-50" />
                                        </button>
                                    }
                                />
                                <Select 
                                    ...
                                    trigger={
                                        <button type="button" className="w-full flex items-center justify-between border border-input rounded-md px-3 py-2 text-sm bg-white hover:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all h-[38px]">
                                            <span className={localEndYear ? 'text-foreground' : 'text-muted-foreground truncate'}>{localEndYear || 'Select Year'}</span>
                                            <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0 opacity-50" />
                                        </button>
                                    }
                                />
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-border mt-auto">
                            <button onClick={handleClear} className="text-sm font-medium text-muted-foreground hover:text-foreground underline underline-offset-2 transition-colors">Clear Date</button>
                            <button onClick={handleApply} className="h-9 text-sm font-bold bg-primary text-white px-4 py-2 rounded-md hover:bg-primary/90 transition-colors shadow-sm">Apply Filter</button>
                        </div>
                    </div>
                </div>
            )}
...
export const StatusDropdown = ({ value, options, onChange, settings }: any) => {
    ...
    return (
        ...
        >
            <div onClick={() => setIsOpen(!isOpen)} className="cursor-pointer hover:opacity-80 transition-all active:scale-95 flex items-center">
                ...
                <div className="absolute top-full left-0 mt-2 min-w-[140px] bg-white border border-border rounded-xl shadow-xl z-50 overflow-hidden flex flex-col max-h-[300px] animate-in fade-in zoom-in-95 duration-100">
                    <div className="overflow-y-auto p-1 custom-thin-scroll">
                        {options.map((opt: string) => (
                            ...
                            >
                                <span className="font-medium">{getSettingBadge('serviceStatuses', opt, settings)}</span>
                            </button>
                        ))}
                    </div>
                </div>
