...
export default function GlobalSearch() {
    ...
    const formatCurrency = (val: number | string | undefined) => {
        ...
        return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num || 0);
    };
    ...
    if (hasEnoughChars) {
        ...
        for (const p of projects) {
            ...
            if (pName.includes(activeQuery) || pAssignee.includes(activeQuery) || pClients.includes(activeQuery)) {
                pMatches.push(p);
            }
        }
        ...
        for (const s of services) {
            ...
            if (sName.includes(activeQuery) || sClient.includes(activeQuery) || sInvoice.includes(activeQuery)) {
                sMatches.push(s);
            }
        }
    }
    ...
    return (
        <div className="relative max-w-[500px] hidden md:block flex-grow" ref={containerRef} id="global-search-container">
            <Search className={`w-4 h-4 absolute left-2.5 top-1/2 -translate-y-1/2 transition-colors ${isOpen && query.length > 0 ? 'text-primary' : 'text-muted-foreground'}`} />
            <input 
                type="text" 
                role="combobox"
            ...
            {showResults && (
                <div id="global-search-listbox" role="listbox" className="absolute top-full left-0 w-full min-w-[500px] bg-white mt-2 rounded-xl border border-border shadow-xl z-50 max-h-[80vh] overflow-y-auto custom-thin-scroll flex flex-col">
                    {totalResults === 0 ? (
                                ...
                                <div className="mb-1">
                                    ...
                                    <div className="px-2 pt-1">
                                        {matchedClients.map(c => {
                                            ...
                                            globalItemIndex++;
                                            return (
                                            <div key={c.id} role="option" aria-selected={isSelected} onClick={() => handleResultClick('client', c.id)} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors group active:scale-95 ${isSelected ? 'bg-accent' : 'hover:bg-accent'}`}>
                                                <div className="flex flex-col overflow-hidden">
                                                    <span className={`text-sm font-bold transition-colors ${isSelected ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>{c.companyName}</span>
                                                    <span className="text-xs text-muted-foreground font-medium mt-0.5">Manager: {c.accountManager || 'Unassigned'}</span>
                                                </div>
                                ...
                                <div className="mb-1">
                                    ...
                                    <div className="px-2 pt-1">
                                        {matchedProjects.map(p => {
                                            ...
                                            globalItemIndex++;
                                            return (
                                            <div key={p.id} role="option" aria-selected={isSelected} onClick={() => handleResultClick('project', p.id)} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors group active:scale-95 ${isSelected ? 'bg-accent' : 'hover:bg-accent'}`}>
                                                <div className="flex flex-col overflow-hidden pr-3">
                                                    <span className={`text-sm font-bold transition-colors ${isSelected ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>{p.name}</span>
                                                    <span className="text-xs text-muted-foreground font-medium mt-0.5 truncate max-w-[220px]">{(p.clients || []).join(', ') || 'Not Set'}</span>
                                                </div>
                                                <div className="shrink-0 ml-4">
                                ...
                                <div>
                                    ...
                                    <div className="px-2 pt-1">
                                        {matchedServices.map(s => {
                                            ...
                                            globalItemIndex++;
                                            return (
                                            <div key={s.id} role="option" aria-selected={isSelected} onClick={() => handleResultClick('service', s.id)} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-colors group active:scale-95 ${isSelected ? 'bg-accent' : 'hover:bg-accent'}`}>
                                                <div className="flex flex-col overflow-hidden pr-3">
                                                    <span className={`text-sm font-bold transition-colors ${isSelected ? 'text-primary' : 'text-foreground group-hover:text-primary'}`}>{s.name}</span>
