                    ...
                    <div className="flex-1 overflow-y-auto custom-thin-scroll px-6 pb-6 flex flex-col gap-5">
                      ...
                      <div className="flex flex-col gap-3">
                        <div>
                          <Select
                            options={(
                              settings?.clientTypes?.map((t: any) => t.name) || [
                                'Developer',
                                'Sales & Marketing',
                              ]
                      ...
                      <div className="flex flex-col gap-6 pt-6 border-t border-slate-200/60">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {(() => {
                            ...
                            if (attachedProjects.length > 0) {
                              const countsByStatus = attachedProjects.reduce(
                                (acc, p) => {
                                ...
                                <div className="flex flex-col gap-1.5">
                                  {Object.entries(countsByStatus)
                                    ...
                                    .map(([status, count]) => (
                                      <div
                                        key={status}
                                        className="flex items-center justify-between gap-2 text-[13px] font-medium text-slate-700 bg-slate-100/50 hover:bg-slate-100 px-3 py-2 rounded-xl transition-colors"
                                      >
                                        <div className="flex items-center gap-2 truncate">
                                          {(() => {
                                            const s = settings?.statuses?.find(
                                              (x: any) => x.name === status
                                            );
                                            return s?.icon ? (
                                              renderIcon(
                                                s.icon,
                                                'w-3.5 h-3.5 text-slate-400 shrink-0'
                                              )
                                            ) : (
                              ...
                              return (
                                <span className="text-[12px] italic text-slate-400 px-1">None</span>
                              );
                            }
                          })()}
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {(() => {
                            const attachedServices = services.filter((s) =>
                              s.clientIds?.includes(client?.clientId || client?.id)
                            );
                            if (attachedServices.length > 0) {
                              const countsByType = attachedServices.reduce(
                                (acc, s) => {
                                  ...
                                  {Object.entries(countsByType)
                                    ...
                                    .map(([type, count]) => (
                                      ...
                                      >
                                        <div className="flex items-center gap-2 truncate">
                                          {(() => {
                                            const t = settings?.serviceTypes?.find(
                                              (x: any) => x.name === type
                                            );
                                            return t?.icon ? (
                                              renderIcon(
                          ...
                          {activeTab === 'notes' && client && (
                            <TimelineTab
                              ...
                              onSaveNotes={async (updatedNotes: any[]) => {
                                await updateClientRecord(
                                  { ...client, notes: updatedNotes } as any,
                                  {
