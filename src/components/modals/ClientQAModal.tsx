            ...
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              ...
              {(project?.onboarding?.clientQA?.submittedAt || project?.onboarding?.clientQA?.updatedAt) && (
                <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium px-3 py-0.5 bg-slate-100 rounded-full">
                  ...
                  {project?.onboarding?.clientQA?.updatedAt && project?.onboarding?.clientQA?.updatedAt !== project?.onboarding?.clientQA?.submittedAt && <span>Updated: {new Date(project?.onboarding?.clientQA.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="w-full max-w-3xl h-full bg-white md:rounded-2xl md:shadow-lg md:border border-slate-200/60 overflow-hidden flex flex-col">
            {template ? (
              <DynamicForm 
                ...
                submitLabel={project?.onboarding?.clientQA?.status === 'Submitted' ? "Update Response" : "Submit"}
              />
            ) : (
