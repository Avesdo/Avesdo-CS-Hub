            ...
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              ...
              {(existingData?.submittedAt || existingData?.updatedAt) && (
                <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium px-3 py-0.5 bg-slate-100 rounded-full">
                  ...
                  {existingData?.updatedAt && existingData?.updatedAt !== existingData?.submittedAt && <span>Updated: {new Date(existingData.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                </div>
              )}
            </div>
          </div>
          ...
          {!template ? (
            ...
            <div className="flex flex-col h-full overflow-y-auto p-6 md:p-10">
              <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4 mb-8">
                <h3 className="font-bold text-amber-900 mb-1 flex items-center gap-2">Legacy CSAT Response</h3>
                <p className="text-sm">This is an older CSAT response submitted prior to the new CSAT template. It is preserved here for historical records.</p>
              </div>
              <div className="space-y-6">
                {Object.entries(existingData || {}).map(([key, val]) => {
                  if (['status', 'createdAt', 'updatedAt', 'submittedAt'].includes(key)) return null;
                  return (
                    <div key={key} className="bg-slate-50 border border-slate-100 p-4 rounded-xl">
                      <h4 className="text-sm font-semibold text-slate-700 mb-2 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</h4>
                      <p className="text-slate-600 bg-white p-3 rounded-lg border border-slate-200">{String(val)}</p>
                    </div>
                  );
                })}
              </div>
