...
import { X, Smile, AlertCircle, BarChart2, Eye, HelpCircle } from 'lucide-react';
                    ...
                    {supportCsatData !== undefined && typeof supportCsatData === 'object' ? (
                      <>
                        <div className="flex flex-col md:flex-row gap-8">
                          ...
                          <div className="flex-shrink-0 w-full md:w-56 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center relative overflow-hidden group">
                            <div className={`absolute inset-0 bg-gradient-to-b opacity-5 group-hover:opacity-10 transition-opacity duration-500 ${supportCsatData.score >= 80 ? 'from-emerald-500 to-transparent' : supportCsatData.score >= 60 ? 'from-amber-500 to-transparent' : 'from-rose-500 to-transparent'}`}></div>
                            <span className="text-xs font-bold text-slate-400 mb-3 relative z-10">Total Score</span>
                            <div className={`text-7xl font-black tabular-nums tracking-tighter relative z-10 ${getScoreColor(supportCsatData.score)}`}>
                          ...
                          <div className="flex-grow flex flex-col justify-center space-y-6">
                            ...
                            <div className="w-full h-4 bg-slate-200/50 rounded-full overflow-hidden flex shadow-inner">
                              {barWidths.promoters > 0 && (
                                <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${barWidths.promoters}%` }} title="Promoters" />
                              )}
                              {barWidths.passives > 0 && (
                                <div className="h-full bg-amber-400 transition-all duration-1000" style={{ width: `${barWidths.passives}%` }} title="Passives" />
                              )}
                              {barWidths.detractors > 0 && (
                                <div className="h-full bg-rose-500 transition-all duration-1000" style={{ width: `${barWidths.detractors}%` }} title="Detractors" />
                              )}
                            </div>
                        ...
                        {supportCsatData.users && supportCsatData.users.length > 0 && (
                          <div className="mt-4">
                            <h4 className="text-xs font-bold text-slate-400 mb-4 ml-1">Feedback by User</h4>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              {supportCsatData.users.map((user: any, idx: number) => {
                                const totalInteractions = (user.happy || 0) + (user.neutral || 0) + (user.unhappy || 0);
                                let dominantColor = 'bg-slate-300';
                                if (user.happy > user.unhappy && user.happy >= user.neutral) dominantColor = 'bg-emerald-500';
                                else if (user.unhappy > user.happy && user.unhappy >= user.neutral) dominantColor = 'bg-rose-500';
                                else if (user.neutral > user.happy && user.neutral > user.unhappy) dominantColor = 'bg-amber-400';

                                return (
                                  <div key={idx} className="flex items-center justify-between bg-white border border-slate-200/60 shadow-sm rounded-2xl p-4 hover:-translate-y-1 hover:shadow-md hover:border-primary/40 transition-all duration-300 group cursor-default relative overflow-hidden">
                                    
                                    {/* Dominant sentiment accent border */}
                                    <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${dominantColor}`}></div>
                                    ...
                                    <div className="flex-1 min-w-0 pl-2">
                                      ...
                                      <div className="text-[11px] font-medium text-slate-400 mt-0.5">
                                        {totalInteractions} Total Interaction{totalInteractions !== 1 ? 's' : ''}
                                      </div>
                                    </div>
                      ...
                      <div className="flex flex-col items-center justify-center py-16 bg-white rounded-3xl border border-slate-200 border-dashed shadow-sm">
                        ...
                        <p className="text-sm font-medium text-slate-400 mt-1 max-w-sm text-center">
                          {supportCsatData !== undefined ? `Legacy Score: ${supportCsatData}` : 'This client has not logged any Happyfox support feedback yet.'}
                        </p>
                      </div>
                    )}
                  </motion.div>
                  ...
                  >
                    {projectsWithCsat.length > 0 ? (
                      <>
                        ...
                        {(() => {
                          const validProjects = projectsWithCsat.map(p => ({
                            project: p,
                            score: calculateProjectHealth(p, settings).csat
                          })).filter(p => typeof p.score === 'number');

                          const totalScore = validProjects.length > 0 
                            ? Math.round(validProjects.reduce((sum, p) => sum + (p.score as number), 0) / validProjects.length)
                            : 0;

                          const promoters = validProjects.filter(p => (p.score as number) >= 80).length;
                          const passives = validProjects.filter(p => (p.score as number) >= 60 && (p.score as number) < 80).length;
                          const detractors = validProjects.filter(p => (p.score as number) < 60).length;
                          ...
                          return (
                            <>
                              <div className="flex flex-col md:flex-row gap-8">
                                {/* Hero Scorecard */}
                                <div className="flex-shrink-0 w-full md:w-56 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center relative overflow-hidden group">
                                  <div className={`absolute inset-0 bg-gradient-to-b opacity-5 group-hover:opacity-10 transition-opacity duration-500 ${totalScore >= 80 ? 'from-emerald-500 to-transparent' : totalScore >= 60 ? 'from-amber-500 to-transparent' : 'from-rose-500 to-transparent'}`}></div>
                                  <span className="text-xs font-bold text-slate-400 mb-3 relative z-10">Average Score</span>
                                  <div className={`text-7xl font-black tabular-nums tracking-tighter relative z-10 ${getScoreColor(totalScore)}`}>
                                    {totalScore}
                                  </div>
                                </div>
                                ...
                                <div className="flex-grow flex flex-col justify-center space-y-6">
                                  ...
                                  <div className="w-full h-4 bg-slate-200/50 rounded-full overflow-hidden flex shadow-inner">
                                    {obBarWidths.promoters > 0 && (
                                      <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${obBarWidths.promoters}%` }} title="Promoters" />
                                    )}
                                    {obBarWidths.passives > 0 && (
                                      <div className="h-full bg-amber-400 transition-all duration-1000" style={{ width: `${obBarWidths.passives}%` }} title="Passives" />
                                    )}
                                    {obBarWidths.detractors > 0 && (
                                      <div className="h-full bg-rose-500 transition-all duration-1000" style={{ width: `${obBarWidths.detractors}%` }} title="Detractors" />
                                    )}
                                  </div>
                              ...
                              <div className="mt-4">
                                <h4 className="text-xs font-bold text-slate-400 mb-4 ml-1">Feedback by Project</h4>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                      ...
                                      >
                                        ...
                                        <div className="flex-1 min-w-0 pl-2 pr-4">
                                          ...
                                          <div className="mt-1 flex items-center h-4">
                                            ...
                                            <span className={`text-[10px] font-bold text-primary flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ${!isLegacy ? 'group-hover:translate-y-0 translate-y-1' : ''}`}>
                                              <Eye className="w-3 h-3" /> View Form
                                            </span>
                                          </div>
                                        </div>

                                        <div className={`shrink-0 w-12 h-12 rounded-full border-2 flex items-center justify-center font-black text-lg bg-white ${numericScore >= 80 ? 'text-emerald-500 border-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.2)]' : numericScore >= 60 ? 'text-amber-500 border-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.2)]' : 'text-rose-500 border-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.2)]'} ${numericScore >= 80 ? 'bg-emerald-50/50' : numericScore >= 60 ? 'bg-amber-50/50' : 'bg-rose-50/50'}`}>
                                          {score}
                                        </div>
                                      </div>
                                    );
