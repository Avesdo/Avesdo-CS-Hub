  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      {isChurned ? (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden min-h-[280px] flex flex-col items-center justify-center relative">
          ...
          <div className="relative z-10 flex flex-col items-center text-center p-8 max-w-md">
            ...
            <h3 className="text-xl font-bold text-slate-800 tracking-tight mb-2">Client Churned</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <div
            className={`absolute -right-6 -top-6 w-24 h-24 rounded-full blur-3xl opacity-10 group-hover:opacity-20 transition-opacity ${getAuraColor(finVal)}`}
          ></div>
          <div>
            <div className="flex items-center gap-2 mb-4 relative z-10">
              <div className="p-1.5 bg-slate-50 rounded-lg group-hover:bg-primary/5 text-slate-500 group-hover:text-primary transition-colors">
            ...
            <div className="flex flex-col mb-1">
              <span
                className={`text-3xl font-black tabular-nums leading-none transition-colors duration-300 ${getScoreColor(finVal)}`}
              >
                {finVal}
              </span>
              <span
          ...
          <div className="mt-3 relative z-10">
            <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${getProgressColor(finVal)}`}
                style={{ width: `${Math.min(typeof finVal === 'number' ? finVal : 0, 100)}%` }}
              />
            </div>
          </div>
        </motion.div>
