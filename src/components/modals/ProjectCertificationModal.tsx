...
interface ProjectCertificationModalProps {
  project: any;
  onClose: () => void;
}

export default function ProjectCertificationModal({ project, onClose }: ProjectCertificationModalProps) {
  const settings = useAppStore(state => state.settings);
  const templateId = Object.keys(settings?.templates || {}).find(k => settings?.templates[k].name === 'Project Certification') || Object.keys(settings?.templates || {}).find(k => settings?.templates[k].type === 'form');
  ...
  const handleCopyLink = () => {
    const portalUrl = `${window.location.origin}/portal/${project.id}?form=certification`;
  ...
  const handleSave = async (data: Record<string, any>) => {
    ...
    try {
      ...
      await updateProjectRecord({ ...project, onboarding: updatedOnboarding }, {
        successMsg: 'Project Certification submitted successfully.',
        errorMsg: 'Failed to save Project Certification.',
      });
      onClose();
    } catch (e) {
      console.error(e);
        ...
        <div className="bg-white/95 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between flex-wrap gap-4 shrink-0 sticky top-0 z-40">
          <div>
            ...
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              ...
              {(project?.onboarding?.certification?.submittedAt || project?.onboarding?.certification?.updatedAt) && (
                <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium px-3 py-0.5 bg-slate-100 rounded-full">
                  {project?.onboarding?.certification?.submittedAt && <span>Submitted: {new Date(project?.onboarding?.certification.submittedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                  {project?.onboarding?.certification?.updatedAt && project?.onboarding?.certification?.updatedAt !== project?.onboarding?.certification?.submittedAt && <span>Updated: {new Date(project?.onboarding?.certification.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {project?.onboarding?.certification && Object.keys(project.onboarding.certification).length > 0 && (
              <button
                onClick={() => exportFormToCSV('Project Certification', project, project.onboarding.certification, template)}
                className="group inline-flex items-center justify-center gap-2 rounded-lg text-[13px] font-medium whitespace-nowrap transition-all duration-200 border border-transparent bg-slate-100 hover:bg-slate-200 text-slate-700 active:scale-95 hover:-translate-y-0.5 px-4 py-2 h-9 focus:ring-2 focus:ring-slate-400/20 focus:outline-none"
              >
          ...
          <div className="w-full max-w-3xl h-full bg-white md:rounded-2xl md:shadow-lg md:border border-slate-200/60 overflow-hidden flex flex-col">
            {template ? (
              <DynamicForm 
                ...
                submitLabel={project?.onboarding?.certification?.status === 'Submitted' ? "Update Response" : "Submit"}
              />
            ) : (
