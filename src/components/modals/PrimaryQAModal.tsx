...
export default function PrimaryQAModal({ project, onClose }: PrimaryQAModalProps) {
  ...
  const templateId =
    Object.keys(settings?.templates || {}).find(
      (k) => settings?.templates[k].name === 'Primary QA'
  ...
  return (
    <Dialog.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[var(--z-modal-overlay)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 duration-200" />
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] sm:w-[calc(100%-3rem)] max-w-6xl h-[95vh] bg-slate-50 flex flex-col rounded-2xl shadow-2xl z-[var(--z-modal)] overflow-hidden outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 duration-200">
          {/* Header */}
          <div className="bg-white/95 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between flex-wrap gap-4 shrink-0 sticky top-0 z-40">
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium px-3 py-0.5 bg-slate-100 rounded-full">
                    ...
                    {project?.onboarding?.primaryQA?.updatedAt &&
                      project?.onboarding?.primaryQA?.updatedAt !==
                        ...
                        <span>
                          Updated:{' '}
                          {new Date(project?.onboarding?.primaryQA.updatedAt).toLocaleDateString(
                            'en-US',
                            { month: 'short', day: 'numeric', year: 'numeric' }
                          )}
                        </span>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {project?.onboarding?.primaryQA &&
                Object.keys(project.onboarding.primaryQA).length > 0 && (
                  <button
                    onClick={() =>
                      exportFormToCSV('Primary QA', project, project.onboarding.primaryQA, template)
                    }
                    className="group inline-flex items-center justify-center gap-2 rounded-lg text-[13px] font-medium whitespace-nowrap transition-all duration-200 border border-transparent bg-slate-100 hover:bg-slate-200 text-slate-700 active:scale-95 hover:-translate-y-0.5 px-4 py-2 h-9 focus:ring-2 focus:ring-slate-400/20 focus:outline-none"
                  >
            ...
            <div className="w-full max-w-3xl h-full bg-white md:rounded-2xl md:shadow-lg md:border border-slate-200/60 overflow-hidden flex flex-col">
              {template ? (
                ...
                <div className="text-center text-slate-500 py-12">
                  No form template found. Please create one in Settings &gt; Templates named
                  "Primary QA".
                </div>
              )}
            </div>
