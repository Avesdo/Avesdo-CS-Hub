...
const QA_SECTIONS = [
  ...
  { id: 's12', title: 'Section 12: Sales Module' },
...
export default function PrimaryQAModal({ project, onClose }: PrimaryQAModalProps) {
  ...
  const [formData, setFormData] = useState<any>(project?.onboarding?.primaryQA || {});
  ...
  const handleSave = async (isSubmit: boolean = false) => {
    ...
    try {
      ...
      await updateProjectRecord({ ...project, onboarding: updatedOnboarding }, {
        successMsg: isSubmit ? 'Primary QA submitted successfully.' : 'Primary QA progress saved.',
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg transition-colors disabled:opacity-50 shadow-sm"
          >
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-10">
        <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="bg-blue-50 text-blue-800 p-4 rounded-xl border border-blue-200 mb-8 text-sm">
            <strong>Instructions:</strong> Please step through each section below. Your progress can be saved at any time. When all checks are complete, click "Submit QA" to record the results and move the project to the Client QA phase.
          </div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">
                          Has this section been fully reviewed?
                        </label>
                        <select 
                          className="w-full max-w-xs px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                          value={formData[`${section.id}_reviewed`] || ''}
                          onChange={(e) => setFormData({...formData, [`${section.id}_reviewed`]: e.target.value})}
                        >
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">
                          Notes / Items to be changed
                        </label>
                        <textarea 
                          rows={3}
                          className="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 resize-y"
                          placeholder="Enter any notes or required changes here..."
