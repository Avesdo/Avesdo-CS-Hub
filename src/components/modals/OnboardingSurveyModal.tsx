...
export default function OnboardingSurveyModal({ project, onClose }: OnboardingSurveyModalProps) {
  const settings = useAppStore(state => state.settings);
  const templateId = Object.keys(settings?.templates || {}).find(k => settings?.templates[k].name === 'Onboarding Survey') || Object.keys(settings?.templates || {}).find(k => settings?.templates[k].type === 'form');
  ...
  const handleSave = async (data: Record<string, any>) => {
    ...
    try {
      ...
      await updateProjectRecord({ ...project, onboarding: updatedOnboarding }, {
        successMsg: 'Onboarding Survey submitted successfully.',
        errorMsg: 'Failed to save Onboarding Survey.',
      });
      onClose();
    } catch (e) {
      console.error(e);
    ...
    <Dialog.Root open={true} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <div className="bg-white/95 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex items-center justify-between flex-wrap gap-4 shrink-0 sticky top-0 z-40">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Onboarding Survey</h2>
            <div className="flex items-center gap-4 mt-1 flex-wrap">
              <p className="text-sm text-slate-500">Project: {project?.name || 'Unknown'}</p>
              {(project?.onboarding?.survey?.submittedAt || project?.onboarding?.survey?.updatedAt) && (
                <div className="flex items-center gap-3 text-[11px] text-slate-400 font-medium px-3 py-0.5 bg-slate-100 rounded-full">
                  {project?.onboarding?.survey?.submittedAt && <span>Submitted: {new Date(project?.onboarding?.survey.submittedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                  {project?.onboarding?.survey?.updatedAt && project?.onboarding?.survey?.updatedAt !== project?.onboarding?.survey?.submittedAt && <span>Updated: {new Date(project?.onboarding?.survey.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {project?.onboarding?.survey && Object.keys(project.onboarding.survey).length > 0 && (
              <button
                onClick={() => exportFormToCSV('Onboarding Survey', project, project.onboarding.survey, template)}
                className="group inline-flex items-center justify-center gap-2 rounded-lg text-[13px] font-medium whitespace-nowrap transition-all duration-200 border border-transparent bg-slate-100 hover:bg-slate-200 text-slate-700 active:scale-95 hover:-translate-y-0.5 px-4 py-2 h-9 focus:ring-2 focus:ring-slate-400/20 focus:outline-none"
              >
          ...
          <div className="w-full max-w-3xl h-full bg-white md:rounded-2xl md:shadow-lg md:border border-slate-200/60 overflow-hidden flex flex-col">
            {template ? (
              <DynamicForm 
                template={template} 
                readOnly={!isEditing} 
                initialValues={project?.onboarding?.survey || {}} 
                onSubmit={handleSave} 
                onCancel={onClose}
                projectFeatures={project.features || []}
                submitLabel={project?.onboarding?.survey?.status === 'Submitted' ? "Update Response" : "Submit Survey"}
              />
            ) : (
              <div className="text-center text-slate-500 py-12">
                No form template found. Please create one in Settings &gt; Templates named "Onboarding Survey".
              </div>
            )}
          </div>
        </div>
