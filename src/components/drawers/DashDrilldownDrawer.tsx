    ...
    <>
      ...
      >
        <div className="px-6 py-5 border-b border-border bg-slate-50 flex items-start justify-between shrink-0">
          ...
          <button onClick={closeDrawer} className="p-2 text-muted-foreground hover:text-foreground hover:bg-accent rounded-md transition-all duration-200 active:scale-95 hover:-translate-y-1 hover:shadow-md shadow-sm shrink-0 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {projects && projects.length > 0 ? (
            [...projects].sort((a: any, b: any) => {
              if (contextType === 'featureAdoption') {
                  const allSysFeats = Array.from(new Set((globalProjects || []).flatMap((proj:any) => proj.features || [])));
              ...
              if (adoptionRate >= 80) adoptionClasses = "bg-emerald-50 text-emerald-600 border-emerald-200";
              else if (adoptionRate >= 50) adoptionClasses = "bg-amber-50 text-amber-600 border-amber-200";
              ...
              return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <h3 className="font-bold text-sm text-foreground group-hover:text-primary transition-colors truncate pr-2" title={p.name}>{p.name}</h3>
                    </div>
                    <p className="text-xs text-muted-foreground truncate font-medium" title={clientDisplay}>{clientDisplay || 'Unknown Client'}</p>
                      ...
                      <div className="flex flex-col gap-1 items-start justify-center shrink-0">
                        {contextType === 'featureAdoption' ? (
                            ...
                            <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground whitespace-nowrap">
                                ...
                                {p.releaseDateVal ? new Date(p.releaseDateVal).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'No Date'}
                            </div>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap justify-end">
                        {contextType === 'featureAdoption' ? (
                          <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold border shadow-sm ${adoptionClasses}`}>
                            <Target className="w-3 h-3 mr-1 inline-block shrink-0" />{adoptionRate}% Adoption
                          </span>
                        ) : contextType === 'managerWorkloadActive' ? (
                          (() => {
                              const primaryClientName = p.clients && p.clients.length > 0 ? p.clients[0] : null;
                              const primaryClient = clients?.find((c:any) => c.companyName === primaryClientName);
              ...
              <div className="bg-slate-50/50 rounded-xl p-8 flex flex-col items-center justify-center text-center border border-dashed border-border mt-4 mx-2">
                <p className="text-sm font-medium text-muted-foreground">No relevant projects found.</p>
              </div>
            )}
          </div>
        </div>
