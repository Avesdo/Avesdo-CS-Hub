...
export default function ClientServicesTab({ client }: ClientServicesTabProps) {
    ...
    // Dynamically generate tabs from settings.serviceTypes, defaulting if empty
    ...
    return (
        <div className="flex flex-col space-y-4">
            <div className="flex justify-between items-end mb-1">
                <div>
                    <h3 className="font-semibold text-foreground text-sm">Client Services</h3>
                    <p className="text-xs text-muted-foreground mt-1">All service history logged to this client.</p>
                </div>
            </div>
            ...
            <div className="flex flex-col space-y-3 pt-2" id="drawerServiceList">
                {displayedServices.length === 0 ? (
                    <div className="border border-dashed border-border rounded-xl bg-slate-50/50 px-6 py-12 flex flex-col items-center justify-center text-center shadow-sm">
                        ...
                        <p className="text-sm text-slate-500 font-medium">No services found for this filter.</p>
                    </div>
                ) : (
                    displayedServices.map((s: any) => {
                        const isAttached = s.projectName && s.projectName !== 'Unknown Project' && s.projectName !== 'No Project';
                        const contextText = isAttached ? s.projectName : 'Client Level';
                        ...
                        const IconElement = serviceTypeDef.icon ? renderIcon(serviceTypeDef.icon, "w-4 h-4") : <Briefcase className="w-4 h-4" />;
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold text-slate-800 text-sm leading-tight tracking-tight group-hover:text-primary transition-colors duration-200">{s.name}</span>
                                            {s.type === 'Additional' && (() => {
                                                const displayStatus = (s.outcome && s.outcome !== 'Pending') ? s.outcome : (s.status || s.outcome);
                                                if (!displayStatus) return null;
                                                
                                                const statusDef = settings?.settingsData?.find((item: any) => 
                                                    (item.category === 'ServiceOutcome' || item.category === 'ServiceStatus') && 
                                                ...
                                                return (
                                                    <span className="px-1.5 py-0.5 text-[10px] font-bold tracking-wider rounded-sm bg-slate-100 text-slate-600" title={displayStatus}>
                                                        {displayStatus}
                                                    </span>
                                                );
                                            })()}
                                        </div>
                                        <span className="text-xs text-slate-500 font-medium mt-0.5">{contextText}</span>
                                    </div>
                                </div>
                                
                                <div className="flex flex-col items-end text-right">
                                    {s.type !== 'Included' && (
                                        <span className="font-bold text-slate-800 text-sm leading-tight">{formatCurrency(s.price || s.cost || s.value || 0)}</span>
                                    )}
                                    <span className="text-xs text-slate-500 font-medium mt-0.5">{s.dateStr || 'TBD'}</span>
                                </div>
                            </div>
                        );
                    })
