    ...
    datasets: [
      {
        ...
        borderColor:
          ...
          '#00bdd9', // Brand color matching Project drawer
      ...
      <div className="flex flex-col space-y-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="inline-flex items-center rounded-lg p-[3px] text-muted-foreground bg-slate-100 w-max justify-start flex-nowrap h-9 shrink-0">
            <button
              ...
              className={`relative inline-flex h-full items-center justify-center gap-1.5 rounded-md px-3 py-1 text-[13px] font-medium whitespace-nowrap transition-all duration-200 active:scale-95 ${filter === '30' ? 'bg-white text-foreground shadow-sm' : 'hover:text-foreground'}`}
            >
              30D
            </button>
            <button
              onClick={() => setFilter('90')}
              className={`relative inline-flex h-full items-center justify-center gap-1.5 rounded-md px-3 py-1 text-[13px] font-medium whitespace-nowrap transition-all duration-200 active:scale-95 ${filter === '90' ? 'bg-white text-foreground shadow-sm' : 'hover:text-foreground'}`}
            >
              90D
            </button>
            <button
              onClick={() => setFilter('365')}
              className={`relative inline-flex h-full items-center justify-center gap-1.5 rounded-md px-3 py-1 text-[13px] font-medium whitespace-nowrap transition-all duration-200 active:scale-95 ${filter === '365' ? 'bg-white text-foreground shadow-sm' : 'hover:text-foreground'}`}
            >
              1Y
            </button>
            <button
              onClick={() => setFilter('all')}
              className={`relative inline-flex h-full items-center justify-center gap-1.5 rounded-md px-3 py-1 text-[13px] font-medium whitespace-nowrap transition-all duration-200 active:scale-95 ${filter === 'all' ? 'bg-white text-foreground shadow-sm' : 'hover:text-foreground'}`}
            >
              All
            </button>
          </div>
        ...
        <div className="bg-slate-50 border border-dashed border-border rounded-xl p-8 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            There is no historical health data for this timeframe.
          </p>
        </div>
      </div>
    );
            ...
            {tableData.map((h, i) => {
              ...
              return (
                <tr key={i} className={`${rowBg} hover:opacity-90 transition-opacity`}>
                  ...
                  <td className={`px-4 py-3 text-right ${diffColor}`}>{diffText}</td>
                </tr>
              );
            })}
          </tbody>
