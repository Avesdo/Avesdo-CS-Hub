...
const forms = [
  {
    ...
    icon: FileText,
    desc: 'Initial onboarding requirements',
  },
  {
    id: 'deliverables',
    flag: 'hasDeliverables',
    title: 'Deliverables Checklist',
    icon: ClipboardList,
    desc: 'Provide requested project deliverables',
  },
  {
    id: 'clientQA',
    flag: 'hasPrimaryQA',
    title: 'Client QA',
    icon: ShieldCheck,
    desc: 'Sign off on client QA review',
  },
  {
    id: 'certification',
    flag: 'hasCertification',
    title: 'Project Certification',
    icon: Award,
    desc: 'Final project completion sign-off',
  },
  {
    id: 'released',
    flag: 'hasReleased',
    title: 'Released',
    icon: ExternalLink,
    desc: 'Project released to live environment',
  },
];
...
export function PortalDashboard({
  ...
  const assignedForms = forms.filter((form) => {
    ...
    const dataNode =
      form.id === 'deliverables'
        ? project.deliverables
        : project.onboarding?.[form.id as keyof typeof project.onboarding];
    return (
      project[form.flag as keyof Project] === true || (dataNode && Object.keys(dataNode).length > 0)
    );
  });
      ...
      if (dTemplate?.sections && dataNode) {
        ...
        dTemplate.sections.forEach((section: any) => {
          ...
          section.items.forEach((item: any) => {
            ...
            total++;
            if ((dataNode[item.id]?.status || item.status || 'Pending') === 'Completed')
              completed++;
          });
        });
  ...
  const progressPercentage =
    forms.length > 0 ? Math.round((completedCount / forms.length) * 100) : 0;
        ...
        <div className="relative z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 shadow-sm">
          <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="text-[15px] font-semibold text-slate-500">Client Portal</div>
            </div>
          </div>
        </div>
                ...
                return (
                  <motion.button
                    ...
                    className={`flex items-center text-left p-6 bg-white border rounded-2xl transition-all duration-300 relative overflow-hidden shadow-sm ${
                      isLocked
                        ? 'opacity-70 grayscale cursor-not-allowed border-slate-200'
                        : 'border-slate-200 hover:border-primary/40 hover:shadow-xl hover:-translate-y-1 group'
                    }`}
                  >
                    ...
                    <div
                      className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 mr-6 transition-all duration-300 ${
                        isLocked
                    ...
                    <div className="flex-1">
                      <h3
                        className={`text-lg font-bold mb-1.5 flex items-center gap-2 transition-colors ${isLocked ? 'text-slate-500' : 'text-slate-800 group-hover:text-primary'}`}
                      >
                        {form.title}
                      </h3>
                      <p className="text-sm text-slate-500 line-clamp-1">{form.desc}</p>

                      {(dataNode?.submittedAt ||
                        (dataNode?.updatedAt &&
                          (form.id === 'deliverables' || dataNode?.status !== 'Draft'))) && (
                          ...
                          {isCompleted ? (
                            <>
                              <span className="inline-block text-[11px] font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 w-fit">
                                Completed:{' '}
                                {new Date(
                                  dataNode.submittedAt || dataNode.updatedAt
                              ...
                              {dataNode?.updatedAt &&
                                dataNode.updatedAt !== dataNode.submittedAt &&
                                (!dataNode?.submittedAt ||
                                  new Date(dataNode.updatedAt) >
                                    new Date(dataNode.submittedAt)) && (
                                  <span className="inline-block text-[11px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100 w-fit">
                                    Updated:{' '}
                                    {new Date(dataNode.updatedAt).toLocaleDateString('en-US', {
                                      month: 'short',
                                      day: 'numeric',
                                      year: 'numeric',
                                    })}
