...
export const RecentUploadActivity: React.FC<RecentUploadActivityProps> = ({ logs, setViewingUploadLog }) => {
      ...
      {uploadLogs.length === 0 ? (
        ...
        <div className="divide-y divide-border overflow-y-auto custom-thin-scroll max-h-[600px]">
          {uploadLogs.slice(0, 30).map((log) => (
            <div key={log.id} className="flex flex-col gap-2 p-3 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex items-center justify-between mt-1 pl-8">
                <div className="flex items-center gap-3 text-[10px] text-slate-500">
                  <span className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-slate-700">{log.autoProcessed?.length || 0}</span> auto
                  </span>
                  <span className="flex items-center gap-1">
                    <AlertCircle className="w-3 h-3 text-amber-500" />
                    <span className="font-bold text-slate-700">{log.sentForReview || 0}</span> review
                  </span>
                </div>
