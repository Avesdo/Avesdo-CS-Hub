...
export function UserAccessManager() {
  ...
  const handleToggleAccountManager = async (u: AppUser) => {
    try {
      await updateUserRecord(u.uid, { isAccountManager: !u.isAccountManager });
    } catch (err) {
      // Toast handled in service
    }
  };

  const handleColorChange = async (uid: string, color: string) => {
    try {
      await updateUserRecord(uid, { color });
    } catch (err) {
      // Toast handled in service
    }
  };
                      ...
                      {u.isAccountManager && (
                        <div className="relative ml-1">
                          <CustomPicker
                            ...
                            onChange={(color) => handleColorChange(u.uid, color)}
                          />
                        </div>
                      )}
                    </div>
