...
export function SortableFieldItem({
      ...
      >
        ...
        <div className="relative flex items-center justify-center">
          <div className="absolute inset-0 flex items-center" aria-hidden="true">
            <div
              className={`w-full border-t-[2px] border-dashed ${isSelected ? 'border-primary-200' : 'border-slate-200'}`}
            />
          </div>
          <div
            className={`relative flex items-center bg-white border-[2px] shadow-sm rounded-full px-2 py-1.5 transition-colors ${isSelected ? 'border-primary-400 ring-4 ring-primary-500/10' : 'border-slate-200 hover:border-slate-300'}`}
          >
            <div
              {...attributes}
              {...listeners}
            ...
            <div className="flex items-center gap-2 pr-4">
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                >
                  <div
                    className={`absolute top-[2px] left-[2px] w-[14px] h-[14px] bg-white rounded-full transition-transform shadow-sm ${field.logicEnabled ? 'translate-x-[14px]' : 'translate-x-0'}`}
                  />
                  <input
                    type="checkbox"
                    className="hidden"
            ...
            <div className="flex items-center gap-2 px-4">
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                >
                  <div
                    className={`absolute top-[2px] left-[2px] w-[14px] h-[14px] bg-white rounded-full transition-transform shadow-sm ${field.featureLogicEnabled ? 'translate-x-[14px]' : 'translate-x-0'}`}
                  />
                  <input
                    type="checkbox"
                    className="hidden"
  ...
  return (
    <div className="relative w-full">
      ...
      >
        <div
          className="px-5 py-3.5 flex-1 flex flex-col gap-1.5 min-w-0"
          onClick={(e) => {
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`p-1.5 rounded-lg shrink-0 transition-colors ${isSelected ? 'bg-primary-50 text-primary-500' : 'bg-slate-100 text-slate-500'}`}
            >
                      ...
                      <>
                        <div className="flex items-center gap-2 pl-2">
                          ...
                          <label className="flex items-center cursor-pointer group">
                            <div
                              className={`w-8 h-[18px] rounded-full relative transition-colors ${field.required ? 'bg-primary' : 'bg-slate-300 group-hover:bg-slate-400'}`}
                            >
                              <div
                                className={`absolute top-[2px] left-[2px] w-3.5 h-3.5 bg-white rounded-full transition-transform shadow-sm ${field.required ? 'translate-x-[14px]' : 'translate-x-0'}`}
                              />
                              <input
                                type="checkbox"
                                className="hidden"
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <label className="flex items-center cursor-pointer group">
                            <div
                              className={`w-8 h-[18px] rounded-full relative transition-colors ${field.logicEnabled ? 'bg-primary' : 'bg-slate-300 group-hover:bg-slate-400'}`}
                            >
                              <div
                                className={`absolute top-[2px] left-[2px] w-3.5 h-3.5 bg-white rounded-full transition-transform shadow-sm ${field.logicEnabled ? 'translate-x-[14px]' : 'translate-x-0'}`}
                              />
                              <input
                                type="checkbox"
                                className="hidden"
                                ...
                                onChange={(e) => {
                                  ...
                                  handleUpdateField(index, {
                                    logicEnabled: enabled,
                                    dependsOn: enabled
                                      ? { fieldId: '', condition: 'equals', value: '' }
                                      : null,
                                  });
                                }}
                              />
            ...
            field.description.trim() !== '' && (
              <div className="pl-10 pr-4 opacity-80 pointer-events-none -mt-1">
                <div
                  ...
                  className="text-[13px] text-slate-500 prose-sm max-w-none prose-p:my-0 prose-p:leading-snug"
                />
              </div>
            )}
          ...
          {!isSelected &&
            ...
            ((field.options && field.options.length > 0) || field.allowOther) && (
                    ...
                    return (
                      <div className="flex flex-col gap-2">
                        {displayOptions.slice(0, 3).map((opt: string, i: number) => (
                          <div
                            key={i}
                            className="text-[13px] text-slate-600 px-3 py-2 rounded-lg border border-slate-200 bg-slate-50/50 leading-relaxed"
                          >
                            {opt || 'Empty Option'}
                          </div>
                        ))}
                  ...
                  return (
                    <div className="flex flex-wrap gap-2">
                      {displayOptions.slice(0, 4).map((opt: string, i: number) => (
                        <div
                          key={i}
                          className="text-[12px] font-medium text-slate-700 px-3 py-1.5 rounded-md border border-slate-200 bg-white shadow-sm flex items-center"
                        >
                          {opt || 'Empty Option'}
                        </div>
                      ))}
