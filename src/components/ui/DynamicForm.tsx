  ...
  const renderField = (field: FormField) => {
    if (readOnly) {
      let displayValue: React.ReactNode = (
        <span className="text-slate-400 italic text-[13px]">Unanswered</span>
      );
      ...
      if (
        ...
        if (Array.isArray(value)) {
          ...
          displayValue = (
            <span className="text-slate-900 text-[15px]">Other: {otherValue || ''}</span>
          );
        } else {
          displayValue = (
      ...
      return (
        <div className="mt-1.5 pl-1 border-l-2 border-primary/20">
          <div className="pl-3">{displayValue}</div>
        </div>
      );
    }
            ...
            render={({ field: { onChange, value } }) => (
              <div>
                <Select
                  ...
                  trigger={
                    <div
                      className={`flex items-center justify-between w-full h-10 px-3 rounded-md border bg-white text-sm transition-all shadow-sm ${
                        errors[field.id]
      ...
      case 'radio':
        return (
          <div className="grid grid-cols-1 gap-2.5 mt-2">
            {(field.options || [])
              ...
              .map((opt) => {
                ...
                return (
                  <label
                    key={opt}
                    className={`relative flex items-center p-3.5 cursor-pointer rounded-xl border transition-all duration-200 group ${isSelected ? 'bg-primary/5 border-primary shadow-sm ring-1 ring-primary/10' : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}
                  >
                    <input
                      type="radio"
                      value={opt}
                    ...
                    />
                    <div
                      className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mr-3.5 transition-colors ${isSelected ? 'border-primary bg-primary' : 'border-slate-300 group-hover:border-slate-400 bg-white'}`}
                    >
                      {isSelected && (
                        <div className="w-2 h-2 rounded-full bg-white animate-in zoom-in" />
                      )}
                    </div>
                    <span
            ...
            {field.allowOther && (
              <label
                className={`relative flex items-center p-3.5 cursor-pointer rounded-xl border transition-all duration-200 group ${formValues[field.id] === '__other__' ? 'bg-primary/5 border-primary shadow-sm ring-1 ring-primary/10' : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}
              >
                <input
                  type="radio"
                  value="__other__"
                ...
                />
                <div
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mr-3.5 transition-colors ${formValues[field.id] === '__other__' ? 'border-primary bg-primary' : 'border-slate-300 group-hover:border-slate-400 bg-white'}`}
                >
      ...
      case 'checkbox':
        return (
          <div className="grid grid-cols-1 gap-2.5 mt-2">
              ...
              .map((opt) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div
                      className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 mr-3.5 transition-colors ${isSelected ? 'border-primary bg-primary' : 'border-slate-300 group-hover:border-slate-400 bg-white'}`}
                    >
                      {isSelected && (
            ...
            {field.allowOther && (
              <label
                className={`relative flex items-center p-3.5 cursor-pointer rounded-xl border transition-all duration-200 group ${(formValues[field.id] || []).includes('__other__') ? 'bg-primary/5 border-primary shadow-sm ring-1 ring-primary/10' : 'bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}
              >
                <input
                  type="checkbox"
                  value="__other__"
                ...
                />
                <div
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 mr-3.5 transition-colors ${(formValues[field.id] || []).includes('__other__') ? 'border-primary bg-primary' : 'border-slate-300 group-hover:border-slate-400 bg-white'}`}
                >
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <DatePicker
                ...
                trigger={
                  <div
                    className={`flex items-center w-full h-10 px-3 gap-2 rounded-md border bg-white text-sm transition-all shadow-sm cursor-pointer ${
                      errors[field.id]
                        ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20'
                        : 'border-slate-200 hover:border-slate-300 focus:border-primary focus:ring-1 focus:ring-primary/20'
                    }`}
      ...
      {visiblePages.length > 1 && (
        <div className="flex items-center gap-1.5 px-6 sm:px-8 pt-8 pb-4 bg-white/95 backdrop-blur-md z-10 shrink-0 border-b border-slate-100/50">
          {visiblePages.map((_, idx) => (
            <div
              key={idx}
              className="flex-1 h-2 rounded-full bg-slate-100 overflow-hidden shadow-inner"
            >
              <motion.div
                initial={false}
