...
import { Trash2, CheckCircle2, Flag, Calendar, User, MessageSquare, ListTodo, FileText } from 'lucide-react';
...
const STATUS_OPTIONS = [
  'Pending', 'Additional Pending', 'Provided', 'Received', 'Question',
  'Delayed', 'In Progress', 'Draft Complete', 'Completed', 'N/A'
];
          ...
          {status !== 'Completed' && (
            <div className="flex flex-col gap-2">
               ...
               {(readOnly || isHidden || isClientPortal) ? (
                 <div className={`w-fit inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border shadow-sm text-[13px] font-bold cursor-default opacity-80 ${getPriorityColor(priority)}`}>
                 ...
                 <div className="w-fit">
                   <Select
                      ...
                      trigger={
                         <button className={`w-fit inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border shadow-sm text-[13px] font-bold transition-all focus:ring-2 focus:ring-primary/20 focus:outline-none hover:brightness-95 active:scale-95 ${getPriorityColor(priority)}`}>
                           <Flag className="w-4 h-4" />
                           {priority}
                         </button>
                      }
