import { collection, onSnapshot, doc, setDoc, getDoc, getDocs, deleteDoc, updateDoc, writeBatch, query, limit, where } from 'firebase/firestore';
...
export function setupRealtimeListeners(onUpdate: (state: AppState) => void) {
    let state: AppState = {
        ...
        ready: { settings: false, clients: false, projects: false, services: false, user: false, aliases: false }
    };
    
    const checkReady = () => {
        if(state.ready.settings && state.ready.clients && state.ready.projects && state.ready.services && state.ready.user && state.ready.aliases) {
    ...
    const unsubSettings = onSnapshot(doc(db, 'settings', 'global_config'), async (snap) => {
        ...
        if (!state.settings) {
            state.settings = {
                managers: [{name: "Roell P", color: "indigo", icon: "User"}, {name: "Jason H", color: "indigo", icon: "User"}, {name: "Rashi G", color: "indigo", icon: "User"}, {name: "Elton P", color: "indigo", icon: "User"}],
                clientTypes: [{name: "Developer", color: "blue", icon: "Building2"}, {name: "Sales & Marketing", color: "blue", icon: "Megaphone"}],
                ...
                statuses: [{ name: "Onboarding", color: "blue", icon: "Play" }, { name: "Active", color: "green", icon: "Activity" }, { name: "Suspended", color: "red", icon: "AlertTriangle" }, { name: "Closed", color: "slate", icon: "Archive" }],
                timelines: [{ name: "Not Started", color: "slate", icon: "Circle" }, { name: "Indefinitely Delayed", color: "fuchsia", icon: "PauseCircle" }, { name: "On Schedule", color: "green", icon: "CheckCircle2" }, { name: "Possibly Delayed", color: "orange", icon: "AlertCircle" }, { name: "Delayed", color: "red", icon: "XCircle" }, { name: "Released", color: "blue", icon: "Rocket" }],
                phases: [{ name: "Not Started", color: "slate", icon: "Circle" }, { name: "Onboarding Email Sent", color: "sky", icon: "Mail" }, { name: "Onboarding Survey Received", color: "sky", icon: "FileText" }, { name: "Awaiting Inputs", color: "purple", icon: "Hourglass" }, { name: "Setup In Progress", color: "purple", icon: "Wrench" }, { name: "Primary QA", color: "fuchsia", icon: "Search" }, { name: "Client QA", color: "fuchsia", icon: "Users" }, { name: "Secondary QA", color: "fuchsia", icon: "SearchCheck" }, { name: "Project Certification", color: "indigo", icon: "ShieldCheck" }, { name: "Released", color: "indigo", icon: "Rocket" }],
    ...
    const unsubUser = onSnapshot(doc(db, 'settings', 'user_profile'), (snap) => {
        state.user = snap.exists() ? snap.data() : { name: "Corporate Hub User", email: "corporate@avesdo.com", initials: "CH" };
...
export async function getHealthHistory(entityId?: string) {
    try {
        ...
        if (historySnap.exists()) {
            ...
            return entityId ? (data.historyMap ? (data.historyMap[entityId] || []) : []) : (data.historyMap || {});
        } else {
            return entityId ? [] : {};
        }
    } catch (err) {
...
export async function updateClientRecord(client: Client, config?: ToastConfig | boolean, logMsg?: string, author?: string) {
    const silent = typeof config === 'boolean' ? config : config?.silent;
    const successMsg = typeof config === 'object' && config.successMsg ? config.successMsg : `All updates to '${client.companyName}' saved successfully.`;
    const errorMsg = typeof config === 'object' && config.errorMsg ? config.errorMsg : `Failed to save updates to '${client.companyName}'.`;
    ...
    try {
        ...
        if (logMsg) {
            ...
            addGlobalLog(logMsg, 'Client', finalClient.clientId || (finalClient as any).id, finalClient.companyName || 'Unknown', author || 'System').catch(console.error);
        }
...
export async function addAutoLog(clientId: string, text: string, author: string = 'System', preventGlobalLog: boolean = false) {
    try {
        ...
        if (!preventGlobalLog) {
            await addGlobalLog(text, 'Client', clientId, clientData.companyName || 'Unknown Client', author);
        }
    } catch (err) {
...
export async function addProjectAutoLog(projectId: string, text: string, author: string = 'System', preventGlobalLog: boolean = false) {
    try {
...
export async function addServiceAutoLog(serviceId: string, text: string, author: string = 'System', preventGlobalLog: boolean = false) {
    try {
...
export async function updateProjectRecord(project: Project, config?: ToastConfig | boolean, logMsg?: string, author?: string, preventGlobalLog: boolean = false) {
    const silent = typeof config === 'boolean' ? config : config?.silent;
    const successMsg = typeof config === 'object' && config.successMsg ? config.successMsg : `All updates to '${project.name}' saved successfully.`;
    const errorMsg = typeof config === 'object' && config.errorMsg ? config.errorMsg : `Failed to save updates to '${project.name}'.`;
    ...
    try {
        ...
        let finalProject = { ...project, history: newHistory };
        if (logMsg) {
            const noteObj = {
                id: crypto.randomUUID(),
                text: logMsg,
            ...
            if (!preventGlobalLog) {
                addGlobalLog(logMsg, 'Project', finalProject.id, finalProject.name || 'Unknown', author || 'System').catch(console.error);
            }
        }
...
export async function updateServiceRecord(service: Service, config?: ToastConfig | boolean, logMsg?: string, author?: string, preventGlobalLog: boolean = false) {
    const silent = typeof config === 'boolean' ? config : config?.silent;
    const successMsg = typeof config === 'object' && config.successMsg ? config.successMsg : `All updates to '${service.name}' saved successfully.`;
    const errorMsg = typeof config === 'object' && config.errorMsg ? config.errorMsg : `Failed to save updates to '${service.name}'.`;
    ...
    try {
        ...
        if (logMsg) {
            ...
            if (!preventGlobalLog) {
                addGlobalLog(logMsg, 'Service', finalService.id || '', finalService.name || 'Unknown', author || 'System').catch(console.error);
            }
        }
...
export async function restoreRecord(collectionName: string, id: string, name: string = 'Record', config?: { silent?: boolean }) {
    try {
...
export async function hardDeleteRecord(collectionName: string, id: string, name: string = 'Record', config?: { silent?: boolean }) {
    try {
...
export async function bulkUpdateProjects(ids: string[], updates: any, config?: { silent?: boolean }) {
    try {
...
export async function bulkUpdateClients(ids: string[], updates: any, config?: { silent?: boolean }) {
    try {
        if (ids.length === 0) return { success: true };
        const batch = writeBatch(db);
        for (const id of ids) {
...
export async function bulkUpdateServices(ids: string[], updates: any, config?: { silent?: boolean }) {
    try {
        if (ids.length === 0) return { success: true };
        const batch = writeBatch(db);
        for (const id of ids) {
...
export async function resolveAlias(aliasId: string, action: 'approve' | 'reject' | 'correct' | 'create_new', customTargetId?: string) {
    try {
        if (action === 'approve') {
            await updateDoc(doc(db, 'aliases', aliasId), { status: 'verified' });
            toast.success("Alias approved and merged.");
        } else if (action === 'correct') {
            await updateDoc(doc(db, 'aliases', aliasId), { status: 'verified', targetId: customTargetId });
