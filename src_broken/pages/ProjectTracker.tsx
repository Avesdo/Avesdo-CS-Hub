...
import { Download, Database, Filter, Target, AlertOctagon, Building, Zap, Calendar, Plus, X, ChevronDown, Clock, CheckCircle2, AlertTriangle, Search, PlayCircle, PauseCircle } from 'lucide-react';
...
export default function ProjectTracker() {
    ...
    useEffect(() => {
        getHealthHistory().then(data => setHealthHistory(data)).catch(err => console.error("Failed to load history", err));
    }, []);
    ...
    const [statusFilter, setStatusFilter] = useState<string[]>(
        location.state?.kpiFilter === 'units' ? ['Active', 'Suspended'] : 
        location.state?.kpiFilter === 'onboarding' ? ['Onboarding'] : 
        []
    );
    ...
    const [releaseDateFilter, setReleaseDateFilter] = useState<{ start: string, end: string } | null>(null);
    ...
    const { onboardingCount, prevOnboardingCount, pipelineCount, prevPipelineCount, riskCount, prevRiskCount, liveUnits, prevLiveUnits } = useMemo(() => {
        ...
        projects.forEach(p => {
            ...
            const finalScore = typeof p.healthScore === 'number' ? p.healthScore : calculateProjectHealth(p, settings).totalScore;
        ...
        return {
            onboardingCount: ob, prevOnboardingCount: prevOb, 
            pipelineCount: pipe, prevPipelineCount: prevPipe,
            riskCount: r, prevRiskCount: prevR, 
            liveUnits: units, prevLiveUnits: prevU
        };
    }, [projects, settings]);
    ...
    const baseProjects = useMemo(() => {
        ...
        return projects.map(p => {
            ...
            if (activeTab === 'Actively Onboarding') {
                return p.projectStatus === 'Onboarding' && !!p.releaseDateVal && p.releaseDateVal <= fortyFiveDays;
            }
    ...
    const filteredProjects = useMemo(() => {
        ...
        if (healthFilter.length > 0) {
            ...
            filtered = filtered.filter(p => {
                ...
                const isWarning = typeof score === 'number' && score >= thresholds.warning && score < thresholds.healthy;
        ...
        if (featuresFilter.length > 0) {
            filtered = filtered.filter(p => p.features && p.features.some((f: string) => featuresFilter.includes(f)));
        }
        ...
        return filtered;
    }, [baseProjects, searchTerm, sortCol, sortAsc, statusFilter, healthFilter, nameFilter, clientFilter, managerFilter, timelineFilter, phaseFilter, featuresFilter, releaseDateFilter]);
    ...
    const handleUpdateProject = async (id: string, field: string, value: any) => {
        ...
        if (p) {
            ...
            else if (field === 'phase') fieldName = 'Phase';
            else if (field === 'timeline') fieldName = 'Timeline';
            ...
            if (field === 'timelineStatus' && value === 'Released') {
                ...
                updates.releaseDateStr = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
            ...
            await updateProjectRecord({ ...p, ...updates }, {
            ...
            if (p.clientIds && (field === 'projectStatus' || updates.projectStatus === 'Active')) {
                ...
                if (statusOld !== statusNew) {
                    for (const cid of p.clientIds) {
                        await addAutoLog(cid, `Project "${p.name}" Status changed from ${statusOld} to ${statusNew}`, user?.name || 'System', true);
                    }
                }
            }
        }
        ...
        try {
            for (const p of projs) {
                ...
                if (p.clientIds && (field === 'projectStatus' || updates.projectStatus === 'Active')) {
                    ...
                    if (statusOld !== statusNew) {
                        for (const cid of p.clientIds) {
                            await addAutoLog(cid, `Project "${p.name}" Status bulk updated from ${statusOld} to ${statusNew}`, user?.name || 'System', true);
                        }
                    }
                }
            }
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden bg-white">
            <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-4 shrink-0 px-4 md:px-6 pt-4">    
                <div>
                    <h1 className="text-2xl md:text-3xl font-semibold text-foreground tracking-tight">Project Tracker</h1>
                    <p className="text-base text-muted-foreground mt-1">Comprehensive tracker and reporting for all projects.</p>
                </div>
                    ...
                    <div className="relative shadow-sm rounded-md" ref={exportMenuRef}>
                        ...
                        {showExportMenu && (
                            <div className="absolute right-0 top-full mt-1 bg-white p-1.5 shadow-xl border border-border rounded-xl min-w-[220px] whitespace-nowrap z-[90]">
                                <div className="px-3 py-2 rounded-md hover:bg-slate-100 cursor-pointer flex items-center gap-2 text-sm font-medium" onClick={() => { setShowExportMenu(false); universalExportCSV('Projects', projects, 'All_Projects'); }}><Database className="w-4 h-4 text-muted-foreground" /> Export All</div>
                                <div className="px-3 py-2 rounded-md hover:bg-slate-100 cursor-pointer flex items-center gap-2 text-sm font-medium" onClick={() => { setShowExportMenu(false); universalExportCSV('Projects', filteredProjects, 'Filtered_Projects'); }}><Filter className="w-4 h-4 text-muted-foreground" /> Export Filtered View</div>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4 shrink-0 px-4 md:px-6">
                <div onClick={() => { clearAllFilters(); setPtFilter('All Projects'); setStatusFilter(['Onboarding']); }} className="cursor-pointer flex flex-col rounded-xl border border-border bg-white/90 backdrop-blur-sm p-6 shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-primary transition-all duration-300 relative overflow-hidden group animate-in fade-in slide-in-from-bottom-4 fill-mode-both active:scale-[0.98]" style={{ animationDelay: '50ms' }}>
                    ...
                    <div className="flex items-start justify-between mb-2 relative z-10">
                        <div className="flex flex-col pr-2">
                            <span className="font-bold text-sm text-foreground flex items-center gap-1.5">Onboarding</span>
                            <span className="text-[11px] text-muted-foreground mt-0.5 leading-snug">Projects currently in onboarding</span>
                        </div>
                ...
                <div onClick={() => { clearAllFilters(); setPtFilter('Actively Onboarding'); }} className="cursor-pointer flex flex-col rounded-xl border border-border bg-white/90 backdrop-blur-sm p-6 shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-primary transition-all duration-300 relative overflow-hidden group animate-in fade-in slide-in-from-bottom-4 fill-mode-both active:scale-[0.98]" style={{ animationDelay: '150ms' }}>
                    ...
                    <div className="flex items-start justify-between mb-2 relative z-10">
                        <div className="flex flex-col pr-2">
                            <span className="font-bold text-sm text-foreground flex items-center gap-1.5">Launch Pipeline</span>
                            <span className="text-[11px] text-muted-foreground mt-0.5 leading-snug">Target launch &le; 45 days</span>
                        </div>
                ...
                <div onClick={() => { clearAllFilters(); setPtFilter('All Projects'); setHealthFilter(['At Risk']); }} className="cursor-pointer flex flex-col rounded-xl border border-border bg-white/90 backdrop-blur-sm p-6 shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-primary transition-all duration-300 relative overflow-hidden group animate-in fade-in slide-in-from-bottom-4 fill-mode-both active:scale-[0.98]" style={{ animationDelay: '250ms' }}>
                    ...
                    <div className="flex items-start justify-between mb-2 relative z-10">
                        <div className="flex flex-col pr-2">
                            <span className="font-bold text-sm text-foreground flex items-center gap-1.5">At-Risk Projects</span>
                            <span className="text-[11px] text-muted-foreground mt-0.5 leading-snug">Live projects scoring under 50</span>
                        </div>
                ...
                <div onClick={() => { clearAllFilters(); setPtFilter('All Released'); setStatusFilter(['Active', 'Suspended']); }} className="cursor-pointer flex flex-col rounded-xl border border-border bg-white/90 backdrop-blur-sm p-6 shadow-sm hover:shadow-md hover:-translate-y-1 hover:border-primary transition-all duration-300 relative overflow-hidden group animate-in fade-in slide-in-from-bottom-4 fill-mode-both active:scale-[0.98]" style={{ animationDelay: '350ms' }}>
                    <div className="absolute -right-6 -top-6 w-24 h-24 bg-blue-500/5 group-hover:bg-blue-500/10 rounded-full blur-xl transition-colors duration-500"></div>
                    <div className="flex items-start justify-between mb-2 relative z-10">
                        <div className="flex flex-col pr-2">
                            <span className="font-bold text-sm text-foreground flex items-center gap-1.5">Total Live Units</span>
                            <span className="text-[11px] text-muted-foreground mt-0.5 leading-snug">Scale of actively supported product</span>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-blue-500/10 text-blue-600 flex items-center justify-center shrink-0 shadow-inner transition-transform duration-300 group-hover:scale-110">
            ...
            <div className="flex-1 min-h-0 flex flex-col px-4 md:px-6 lg:px-8 pb-6 relative z-20 w-full">
                {(() => {
                    ...
                    <div className="flex flex-col gap-3 pb-3 shrink-0 w-full">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto custom-thin-scroll py-2 px-2 -mx-2">
                            <div className="flex items-center bg-slate-100/80 border border-slate-200 rounded-lg p-0.5 shadow-[inset_0_1px_2px_rgba(0,0,0,0.03)] mr-2 shrink-0">
                                <button onClick={() => setViewMode('list')} className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary ${viewMode === 'list' ? 'bg-white text-foreground shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-foreground'}`}>
                                    List View
                                </button>
                                <button onClick={() => setViewMode('calendar')} className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary ${viewMode === 'calendar' ? 'bg-white text-foreground shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-foreground'}`}>
                                    Calendar
                                </button>
                            </div>
                            {viewMode === 'list' && (() => {
                    ...
                    {(nameFilter.length > 0 || clientFilter.length > 0 || managerFilter.length > 0 || timelineFilter.length > 0 || phaseFilter.length > 0 || statusFilter.length > 0 || healthFilter.length > 0 || featuresFilter.length > 0 || releaseDateFilter !== null) && (
                        <ActiveFilterBar 
                            filters={[
                                { label: 'Project Name', values: nameFilter, onRemove: (v) => removeFilterItem(setNameFilter, nameFilter, v) },
                                ...
                                { label: 'Timeline', values: timelineFilter, onRemove: (v) => removeFilterItem(setTimelineFilter, timelineFilter, v) },
                                { label: 'Phase', values: phaseFilter, onRemove: (v) => removeFilterItem(setPhaseFilter, phaseFilter, v) },
                                { label: 'Health', values: healthFilter, onRemove: (v) => removeFilterItem(setHealthFilter, healthFilter, v) },
                                { label: 'Feature', values: featuresFilter, onRemove: (v) => removeFilterItem(setFeaturesFilter, featuresFilter, v) },
                                { 
                                    label: 'Release', 
                                    values: releaseDateFilter ? [releaseDateFilter.start === releaseDateFilter.end ? releaseDateFilter.start : `${releaseDateFilter.start} to ${releaseDateFilter.end}`] : [], 
                    ...
                    const footerContent = viewMode === 'list' ? (
                        <TableFooter 
                            totalItems={filteredProjects.length} 
                            label="Total Projects"
                            rightContent={
                                <span className="text-[11px] capitalize tracking-wider text-muted-foreground font-bold">
                                    Total Units Displayed: <span className="text-foreground text-[13px] font-bold ml-1">{filteredProjects.reduce((sum, p) => sum + (Number(p.units) || 0), 0).toLocaleString()}</span>
                                </span>
                            }
                        />
                    ) : null;
