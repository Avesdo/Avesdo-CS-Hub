...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const currentState = React.useMemo(() => {
    ...
    return {
      ...
      clients: state.clients.map(c => {
        ...
        const activeProjectsCount = cProjects.filter(p => p.projectStatus === 'Active' || p.projectStatus === 'Suspended').length;
        const onboardingProjectsCount = cProjects.filter(p => p.projectStatus === 'Onboarding').length;
        const closedProjectsCount = cProjects.filter(p => p.projectStatus === 'Closed' || p.projectStatus === 'Completed').length;
        return {
          ...c,
          activeProjectsCount,
      ...
      projects: state.projects.map(p => {
        ...
        return {
          ...p,
          clients: resolvedIds.map((id: string) => state.clients.find(c => c.clientId === id || c.id === id)?.companyName).filter(Boolean) as string[]
        };
      }),
      services: state.services.map(s => {
        ...
        if (resolvedIds.length === 0 && s.projectId && s.projectId !== 'N/A') {
            ...
            if (assignedProject) {
                resolvedIds = assignedProject.clientIds || (assignedProject.clientId ? [assignedProject.clientId] : []);
            }
        }

        let resolvedManager = s.manager || s.assignee;
        if ((!resolvedManager || resolvedManager === 'Unassigned' || resolvedManager === 'Not Set' || resolvedManager === 'Unknown') && s.projectId && s.projectId !== 'N/A') {
            const assignedProject = state.projects.find(p => p.id === s.projectId);
            if (assignedProject) {
                resolvedManager = assignedProject.assignee || assignedProject.manager || resolvedManager;
            }
        }
        ...
        return {
          ...
          clients: resolvedIds.map((id: string) => state.clients.find(c => c.clientId === id || c.id === id)?.companyName).filter(Boolean) as string[],
          clientName: resolvedIds.map((id: string) => state.clients.find(c => c.clientId === id || c.id === id)?.companyName).filter(Boolean)[0] || 'Unknown Client'
        };
      })
    };
  }, [state]);

  if (!currentState.ready.settings || !currentState.ready.clients || !currentState.ready.projects || !currentState.ready.services || !currentState.ready.user || !currentState.ready.aliases) {
    return (
  ...
  return (
    <AppStateContext.Provider value={currentState}>
      {children}
