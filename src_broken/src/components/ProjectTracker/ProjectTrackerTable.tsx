...
export const ProjectTrackerTable: React.FC<ProjectTrackerTableProps> = ({
  ...
  const activeManagers = new Set(settings?.managers?.map((m:any) => m.name) || []);
  const allManagers = Array.from(new Set(baseProjects.map(p => p.assignee || 'Unassigned'))).filter(m => m === 'Unassigned' || activeManagers.has(m)).sort();
  
  const activeTimelines = new Set(settings?.timelines?.map((t:any) => t.name) || []);
  const allTimelines = Array.from(new Set(baseProjects.map(p => p.timelineStatus || 'Not Set'))).filter(t => t === 'Not Set' || activeTimelines.has(t)).sort();
  
  const activePhases = new Set(settings?.phases?.map((p:any) => p.name) || []);
  const allPhases = Array.from(new Set(baseProjects.map(p => p.onboardingPhase || 'Not Set'))).filter(p => p === 'Not Set' || activePhases.has(p)).sort();
                      );
                    })()}
                  </td>
                )}
