...
export function ActiveFilterBar({ filters, onClearAll }: ActiveFilterBarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4 animate-in fade-in slide-in-from-top-1 px-1">
      ...
      {activeItems.map((item, idx) => (
        <span key={`${item.groupLabel}-${item.value}-${idx}`} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200">
          ...
          <button 
            ...
            className="hover:text-red-500 transition-colors focus:outline-none focus:ring-2 focus:ring-[#00bdd9]/20 rounded-full"
          >
            <X className="w-3 h-3" />
          </button>
        </span>
      ...
      {onClearAll && activeItems.length > 1 && (
        <button 
          onClick={onClearAll} 
          className="inline-flex items-center gap-1 px-3 py-1 ml-auto text-xs font-semibold text-slate-500 hover:text-red-600 transition-colors bg-white hover:bg-red-50 rounded-full border border-slate-200 hover:border-red-200 focus:outline-none focus:ring-2 focus:ring-red-500/20 shadow-sm"
        >
