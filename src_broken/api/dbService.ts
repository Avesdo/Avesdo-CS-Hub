    ...
    const unsubSettings = onSnapshot(doc(db, 'settings', 'global_config'), async (snap) => {
        ...
        if (!state.settings) {
            state.settings = {
                ...
                timelines: [{ name: "Not Started", color: "slate" }, { name: "Indefinitely Delayed", color: "fuchsia" }, { name: "On Schedule", color: "green" }, { name: "Possibly Delayed", color: "orange" }, { name: "Delayed", color: "red" }, { name: "Released", color: "blue" }],
