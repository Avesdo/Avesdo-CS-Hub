...
import { updateProjectRecord, updateClientRecord, addAutoLog, addProjectAutoLog } from '../../api/dbService';
...
export default function AddProjectModal() {
  ...
  const handleSubmit = async () => {
    ...
    const nameExists = projects.some(p => (p.name || '').toLowerCase() === name.trim().toLowerCase());
    if (nameExists) {
    ...
    try {
      ...
      let finalReleaseDateVal = releaseDateVal || 0;
      if (releaseDateVal) {
          finalReleaseDateStr = new Date(releaseDateVal).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
      }
      ...
      dbPromises.push(updateProjectRecord(newProject, {
      ...
      for (const client of matchedClients) {
        dbPromises.push(addAutoLog(client.clientId || client.id, `New project added: ${newProject.name}`, user?.name || 'System', true));
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-black/40 backdrop-blur-sm z-[120] transition-all duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`}></div>
      <div 
        className={`fixed inset-0 z-[130] overflow-y-auto overflow-x-hidden p-4 sm:p-6 md:py-12 flex items-start justify-center custom-thin-scroll transition-all duration-200 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} 
        onClick={(e) => {
          if (e.target === e.currentTarget) handleClose();
        }}
      >
        <div className={`relative w-full max-w-2xl bg-white border border-border rounded-xl shadow-2xl flex flex-col my-auto text-card-foreground transition-all duration-200 ease-out transform ${isVisible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'} ${activeModal !== 'addProject' ? 'blur-[2px] brightness-[0.8] pointer-events-none' : ''}`}>
          <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-muted/20 rounded-t-xl shrink-0">
            <h3 className="font-semibold text-lg tracking-tight text-foreground">Add New Project</h3>
          ...
          <div className="p-6 flex-1">
            {errorMsg && (
              <div id="ap-error" className="text-destructive text-sm font-semibold bg-destructive/10 px-3 py-2 rounded-md border border-destructive/20 mb-5">
                {errorMsg}
              </div>
            )}
          {/* REQUIRED METADATA */}
          <div className="bg-muted/40 border border-border rounded-lg p-5 space-y-5 mb-5">
            <div>
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">Project Name <span className="text-destructive">*</span></label>
              <input 
                type="text" 
                value={name}
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-[11px] font-semibold text-muted-foreground">Attached Client(s) <span className="text-destructive">*</span></label>
                <button type="button" onClick={() => openModal('addClient')} className="text-[10px] font-bold text-primary hover:underline transition-all active:scale-95 focus-visible:ring-2 focus-visible:ring-primary/20 focus-visible:outline-none rounded-sm px-1">
                  + Add Client
                </button>
              </div>
              <MultiSelect 
                values={selectedClients}
                options={[...clients].sort((a,b) => (a.companyName || a.name || '').localeCompare(b.companyName || b.name || '')).map(c => ({ label: c.companyName || c.name, value: c.clientId || c.id }))}
                onChange={(vals) => { setSelectedClients(vals); setErrorMsg(''); }}
                searchable={true}
                ...
                trigger={
                  <button type="button" className="w-full bg-white shadow-sm h-[38px] active:scale-95 transition-all duration-200 hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20 border border-input rounded-md px-3 text-left flex justify-between items-center text-sm">
                    <span className={`truncate ${selectedClients.length ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {selectedClients.length 
                          ? selectedClients.map(id => clients.find(c => c.clientId === id || c.id === id)?.companyName || id).join(', ') 
          ...
          <div className="bg-muted/40 border border-border rounded-lg p-5 mb-5 grid grid-cols-2 gap-5 items-start">
            <div className="mt-0">
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">Release Date</label>
              <DatePicker 
            ...
            <div className="mt-0 !space-y-0">
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">Live Units <span className="text-destructive">*</span></label>
              <input 
                type="number" 
                value={units}
                onChange={(e) => { setUnits(e.target.value); setErrorMsg(''); }}
          ...
          <div className="bg-muted/40 border border-border rounded-lg p-5 mb-5 grid grid-cols-2 gap-5">
            ...
            <div className="mt-0">
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">Active Features</label>
              <MultiSelect 
                ...
                trigger={
                  <button type="button" className="w-full bg-white shadow-sm h-[38px] active:scale-95 transition-all duration-200 hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20 border border-input rounded-md px-3 text-left flex justify-between items-center text-sm">
                    <span className={`truncate ${activeFeatures.length ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {activeFeatures.length ? `${activeFeatures.length} Selected` : 'Select Active Features...'}
                    </span>
                    <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0" />
                  </button>
                }
          ...
          <div className="bg-muted/40 border border-border rounded-lg p-5 space-y-5">
            <div>
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">Deliverables Checklist URL <span className="text-muted-foreground font-normal">(Optional)</span></label>
            ...
            <div>
              <label className="block text-[11px] font-semibold text-muted-foreground mb-2">KYC Details <span className="text-muted-foreground font-normal">(Optional)</span></label>
              <textarea 
                rows={2} 
                value={kycDetails}
