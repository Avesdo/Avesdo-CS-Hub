...
import { getHealthBadge, getSettingBadge, getSafeHex, hexToRgba, renderIcon } from '../../../utils/uiUtils';
...
export default function ClientProjectsTab({ client }: ClientProjectsTabProps) {
    ...
    const activeProjects = useMemo(() => {
        return clientProjects.filter(p => p.projectStatus === 'Active' || p.projectStatus === 'Suspended')
            .sort((a: any, b: any) => (b.releaseDateVal || 0) - (a.releaseDateVal || 0));
    }, [clientProjects]);

    const onboardingProjects = useMemo(() => {
        return clientProjects.filter(p => p.projectStatus === 'Onboarding')
            .sort((a: any, b: any) => {
    ...
    const closedProjects = useMemo(() => {
        return clientProjects.filter(p => p.projectStatus === 'Closed' || p.projectStatus === 'Completed')
            .sort((a: any, b: any) => (b.releaseDateVal || 0) - (a.releaseDateVal || 0));
    }, [clientProjects]);

    const getDisplayedProjects = () => {
        if (filter === 'All') return [...clientProjects].sort((a: any, b: any) => (b.releaseDateVal || 0) - (a.releaseDateVal || 0));
        ...
        <div className="flex flex-col space-y-4">
            <div className="flex justify-between items-end mb-1">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">All projects associated with this client.</p>
                </div>
            </div>
            ...
            <div className="flex flex-col space-y-3 pt-2" id="drawerProjectList">
                {displayedProjects.length === 0 ? (
                    <div className="border border-dashed border-border rounded-xl bg-slate-50/50 px-6 py-12 flex flex-col items-center justify-center text-center shadow-sm">
                        ...
                        <p className="text-sm text-slate-500 font-medium">No {filter === 'All' ? '' : filter.toLowerCase()} projects found.</p>
                    </div>
                ) : (
                    displayedProjects.map((p: any) => {
                        ...
                        const statusDef: any = settings?.statuses?.find((item: any) => item.name === p.projectStatus) || {};
                        const hex = getSafeHex(statusDef.color, 'slate');
                        const IconElement = statusDef.icon ? renderIcon(statusDef.icon, "w-4 h-4") : <Folder className="w-4 h-4" />;
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex flex-col items-start">
                                        <span className="font-bold text-slate-800 text-sm leading-tight tracking-tight group-hover:text-primary transition-colors duration-200">{p.name}</span>
                                    </div>
                                </div>
