...
import { X, Pencil, Check, ChevronDown, AlertTriangle, Trash2, ArrowLeft, Info } from 'lucide-react';
import { useUI } from '../../context/UIContext';
import { useAppState } from '../../context/AppStateContext';
import { getSettingBadge } from '../../utils/uiUtils';
import { updateProjectRecord, addAutoLog, deleteProjectRecord, addProjectAutoLog, updateServiceRecord, addServiceAutoLog } from '../../api/dbService';
...
export default function ProjectDrawer() {
    ...
    try {
      ...
      if (servicesToUpdate.length > 0) {
          const servicePromises = servicesToUpdate.map(async (s: any) => {
              await updateServiceRecord({ ...s, projectName: newName }, true, `Attached project name updated from "${oldName}" to "${newName}"`, user?.name, true);
          });
      ...
      if (project.clientIds) {
          ...
          await Promise.all(project.clientIds.map(async (cid: string) => {
              await addAutoLog(cid, `Project name updated from "${oldName}" to "${newName}"`, user?.name || 'System', true);
          }));
      }
    } catch (err) {
      setEditNameValue(project.name || '');
  ...
  const handleUpdateStatus = async (val: string) => {
    ...
    await updateProjectRecord({ ...project, status: val as 'Active' | 'On Hold' | 'Completed' | 'Churned' | 'Pipeline' | 'Cancelled' }, {
      successMsg: `Status successfully updated for '${project.name}'.`,
      errorMsg: `Failed to update status for '${project.name}'.`
    }, logMsg, user?.name);

    if (project.clientIds) {
      for (const cid of project.clientIds) await addAutoLog(cid, `Project "${project.name}" Status changed from ${oldVal} to ${val}`, user?.name || 'System', true);
    }
  };
  ...
  const handleUpdateManager = async (val: string) => {
    ...
    }, `Assignee changed from ${oldVal} to ${val}`, user?.name);
  };

  const handleUpdateTimeline = async (val: string) => {
    if (!project || project.timeline === val) return;
    const oldVal = project.timeline || 'On Track';
    await updateProjectRecord({ ...project, timeline: val as 'On Track' | 'At Risk' | 'Delayed' | 'Not Started' }, {
      successMsg: `Timeline successfully updated for '${project.name}'.`,
      errorMsg: `Failed to update timeline for '${project.name}'.`
    }, `Timeline changed from ${oldVal} to ${val}`, user?.name);
  };
      ...
      >
        ...
        {project?.projectStatus === 'Onboarding' && (
          <div className="bg-muted/50 text-muted-foreground border-b border-border px-6 py-3 text-sm font-semibold flex items-center gap-2 shrink-0">
            ...
            <span>This project is currently in Onboarding. Health metrics are paused until it goes Live.</span>
          </div>
        )}

        {(project?.projectStatus === 'Closed' || project?.projectStatus === 'Completed' || project?.projectStatus === 'Cancelled' || project?.projectStatus === 'Churned') && (
          <div className="bg-muted/50 text-muted-foreground border-b border-border px-6 py-3 text-sm font-semibold flex items-center gap-2 shrink-0">
            <Info className="w-5 h-5 shrink-0 text-muted-foreground" />
        ...
        <div className="px-6 py-5 border-b border-border bg-slate-50 flex items-center gap-6 relative shrink-0">
          <div className="gauge-container shrink-0">
            <svg className="gauge-svg" viewBox="0 0 100 100">
              ...
              <circle 
                ...
                style={{ strokeDashoffset: healthScore === 'N/A' ? 0 : 264 - (264 * (healthScore as number)) / 100, stroke: 'currentcolor' }}
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={`leading-none font-bold ${healthScore === 'N/A' ? 'text-slate-400 text-[22px]' : (healthScore as number) < 50 ? 'text-red-600 text-[30px]' : (healthScore as number) < 80 ? 'text-amber-600 text-[30px]' : 'text-lime-600 text-[30px]'}`}>
          ...
          <div className="flex-1 min-w-0 pr-20 flex flex-col justify-center py-2 relative">
            <div className="flex items-center gap-3 group mb-1">
              {isEditingName ? (
                <div className="flex items-center gap-2 w-full">
                  ...
                  />
                  <button onClick={handleUpdateName} className="p-1 hover:bg-emerald-50 rounded-md text-emerald-600 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><Check className="w-5 h-5" /></button>
                  <button onClick={() => { setEditNameValue(project?.name || ''); setIsEditingName(false); }} className="p-1 hover:bg-muted rounded-md text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><X className="w-5 h-5" /></button>
                </div>
              ) : (
                <>
                  <h2 className="text-2xl font-semibold text-foreground tracking-tight leading-tight truncate">{project?.name || 'Loading Project...'}</h2>
                  <button onClick={() => setIsEditingName(true)} className="text-muted-foreground hover:text-primary opacity-0 group-hover:opacity-100 transition-all active:scale-95 shrink-0" title="Edit Name">
            ...
            <span className="text-sm text-muted-foreground mb-2 truncate block">{project?.clients?.join(', ') || 'No clients'}</span>
          ...
          <div className="absolute top-4 right-4 flex items-center gap-1">
            {isConfirmingDelete ? (
              <button onClick={async () => { if (project) { await deleteProjectRecord(project.id, project.name); await addProjectAutoLog(project.id, `Project archived`, user?.name || 'System'); if (project.clientIds) { for (const cid of project.clientIds) await addAutoLog(cid, `Project "${project.name}" archived`, user?.name || 'System', true); } const pServices = services.filter((s: any) => s.projectId === project.id); for (const svc of pServices) await addServiceAutoLog(svc.id, `Attached Project "${project.name}" archived`, user?.name || 'System', true); closeDrawer(); } }} className="flex items-center gap-1.5 px-3 py-1.5 border-2 border-red-300 rounded-md text-red-600 bg-white hover:bg-red-50 font-semibold shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                <AlertTriangle className="w-4 h-4" /> Confirm Archive
              </button>
            ) : (
              <button onClick={() => setIsConfirmingDelete(true)} className="p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive rounded-md transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" title="Archive Project">
                <Trash2 className="w-5 h-5" />
              </button>
            )}
            <button onClick={closeDrawer} className="p-2 text-muted-foreground hover:text-foreground hover:bg-accent rounded-md transition-all active:scale-95 hover:-translate-y-1 hover:shadow-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/20 duration-200" title="Close Drawer">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
