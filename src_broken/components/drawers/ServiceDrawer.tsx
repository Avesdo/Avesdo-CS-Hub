...
import { X, Layers, Pencil, Check, ChevronDown, Trash2, AlertTriangle, PlusCircle, CheckCircle2 } from 'lucide-react';
import { useUI } from '../../context/UIContext';
import { useAppState } from '../../context/AppStateContext';
import { getSettingBadge, COLOR_MAP, hexToRgba, renderIcon } from '../../utils/uiUtils';
import { updateServiceRecord, addAutoLog, addProjectAutoLog, deleteServiceRecord, addServiceAutoLog } from '../../api/dbService';
...
export default function ServiceDrawer() {
  ...
  const clientNames = service?.clientName || (service?.clients?.length ? service.clients.join(', ') : null);
  const projectName = service?.projectName && service.projectName !== 'N/A' ? service.projectName : null;

  const relationalContext = clientNames 
    ? (projectName ? `${clientNames} • ${projectName}` : `${clientNames} • Client Level Service`) 
    : 'Orphaned Service';
  ...
  const handleUpdateName = async () => {
    ...
    try {
      ...
      await updateServiceRecord({ ...service, name: newName }, {
        successMsg: `Service Name successfully updated to '${newName}'.`,
        errorMsg: `Failed to update Service Name to '${newName}'.`
      }, `Service name updated from "${oldName}" to "${newName}"`, user?.name);
      
      if (service.clientIds) {
          for (const cid of service.clientIds) {
              await addAutoLog(cid, `Service name updated from "${oldName}" to "${newName}"`, user?.name || 'System', true);
          }
      }
      if (service.projectId && service.projectId !== 'N/A') {
          await addProjectAutoLog(service.projectId, `Service name updated from "${oldName}" to "${newName}"`, user?.name || 'System', true);
      }
    } catch (err) {
      setEditNameValue(service.name || '');
    }
  ...
  const handleUpdateStatus = async (status: string) => {
    ...
    await updateServiceRecord({ ...service, ...updates }, {
      successMsg: `Status successfully updated for '${service.name}'.`,
      errorMsg: `Failed to update status for '${service.name}'.`
    }, `Status changed from ${oldStatus} to ${status}`, user?.name);
    
    if (status === 'Completed') {
        if (service.clientIds) {
            for (const cid of service.clientIds) {
                await addAutoLog(cid, `Service "${service.name}" status changed from ${oldStatus} to ${status}`, user?.name || 'System', true);
            }
        }
        if (service.projectId && service.projectId !== 'N/A') {
            await addProjectAutoLog(service.projectId, `Service "${service.name}" status changed from ${oldStatus} to ${status}`, user?.name || 'System', true);
        }
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
        ...
        if (service.projectId && service.projectId !== 'N/A') {
            await addProjectAutoLog(service.projectId, `Service "${service.name}" archived`, user?.name || 'System', true);
        }
      ...
      >
        <div className="px-6 py-5 border-b border-border bg-slate-50 flex items-center gap-6 relative shrink-0">
          <div className="shrink-0">
            {(() => {
                const serviceType = settings?.serviceTypes?.find((t: any) => t.name === service?.type);
                const hex = serviceType ? (COLOR_MAP[serviceType.color] || COLOR_MAP.slate) : COLOR_MAP.slate;
                return (
                    <div 
              ...
              <div className="flex items-center gap-3 group mb-1">
                {isEditingName ? (
                  <div className="flex items-center gap-2 w-full">
                    ...
                    <button onClick={() => { setEditNameValue(service?.name || ''); setIsEditingName(false); }} className="p-1 hover:bg-muted rounded-md text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><X className="w-5 h-5" /></button>
                  </div>
                ) : (
                  <>
                    <h2 className="text-2xl font-semibold text-foreground tracking-tight leading-tight truncate">{service?.name || 'Loading Service...'}</h2>
                 ...
                 <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className="relative inline-block">
                      <Select
                         ...
                         options={((settings?.settingsData || []).filter((s:any) => s.category === 'ServiceStatus').map((s: any) => s.name) || []).map((s: any) => ({ label: getSettingBadge('serviceStatuses', s, settings), value: s }))}
          ...
          <div className="absolute top-4 right-4 flex items-center gap-1">
            {isConfirmingDelete ? (
              <button onClick={handleConfirmDelete} className="flex items-center gap-1.5 px-3 py-1.5 border-2 border-red-300 rounded-md text-red-600 bg-white hover:bg-red-50 font-semibold shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                <AlertTriangle className="w-4 h-4" /> Confirm Archive
              </button>
            ) : (
              <button onClick={() => setIsConfirmingDelete(true)} className="p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive rounded-md transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" title="Archive Service">
