  ...
  if (isClosed) {
    return (
      <div className="bg-muted/50 text-muted-foreground border border-border rounded-xl px-6 py-4 text-sm font-semibold flex flex-col items-center gap-2 text-center mt-4 mx-4">
        <Activity className="w-6 h-6 shrink-0 text-muted-foreground mb-1" />
        <span>No active data to score.</span>
        <span className="font-normal text-xs max-w-xs">Metrics are no longer tracked for {project?.projectStatus?.toLowerCase()} projects.</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <details className="bg-white border border-border rounded-xl shadow-sm transition-all duration-300 hover:border-primary/40 group overflow-hidden">
          <summary className="p-5 cursor-pointer outline-none hover:bg-slate-50 flex justify-between items-center list-none [&::-webkit-details-marker]:hidden">
              <div className="flex flex-col pr-4">
                  <span className="mb-1 text-sm font-bold text-foreground tracking-tight">Operational Activity</span>
                  <span className="text-xs text-muted-foreground font-medium leading-snug">Raw volume of events executed in this project.</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                  <div className="w-20 h-2 bg-muted rounded-full overflow-hidden hidden sm:flex">
                      <div className={`h-full transition-all ${getBarColor(opVal)}`} style={{ width: typeof opVal === 'number' ? `${Math.min(opVal, 100)}%` : '0%' }}></div>
                  </div>
                  <span className={`w-8 text-right font-bold text-xl tabular-nums ${getScoreColor(opVal)}`}>{opVal}</span>
          ...
          <div className="px-5 pb-5 pt-4 border-t border-border text-sm text-muted-foreground space-y-3 bg-white">
              <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-foreground">{typeof opVal === 'number' ? opVal : 0}</span>
              </div>
          </div>
      </details>
      ...
      <details className="bg-white border border-border rounded-xl shadow-sm transition-all duration-300 hover:border-primary/40 group overflow-hidden">
          <summary className="p-5 cursor-pointer outline-none hover:bg-slate-50 flex justify-between items-center list-none [&::-webkit-details-marker]:hidden">
              <div className="flex flex-col pr-4">
                  <span className="mb-1 text-sm font-bold text-foreground tracking-tight">Active User Volume</span>
                  <span className="text-xs text-muted-foreground font-medium leading-snug">Unique users successfully logging into this project.</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                  <div className="w-20 h-2 bg-muted rounded-full overflow-hidden hidden sm:flex">
                      <div className={`h-full transition-all ${getBarColor(usrVal)}`} style={{ width: typeof usrVal === 'number' ? `${Math.min(usrVal, 100)}%` : '0%' }}></div>
                  </div>
                  <span className={`w-8 text-right font-bold text-xl tabular-nums ${getScoreColor(usrVal)}`}>{usrVal}</span>
                  <ChevronDown className="w-4 h-4 text-muted-foreground transition-transform group-open:rotate-180" />
              </div>
          </summary>
          <div className="px-5 pb-5 pt-4 border-t border-border text-sm text-muted-foreground space-y-3 bg-white">
              <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-foreground">{typeof usrVal === 'number' ? usrVal : 0}</span>
              </div>
          </div>
      </details>
      ...
      <details className="bg-white border border-border rounded-xl shadow-sm transition-all duration-300 hover:border-primary/40 group overflow-hidden">
          <summary className="p-5 cursor-pointer outline-none hover:bg-slate-50 flex justify-between items-center list-none [&::-webkit-details-marker]:hidden">
              <div className="flex flex-col pr-4">
                  <span className="mb-1 text-sm font-bold text-foreground tracking-tight">Feature Adoption</span>
                  <span className="text-xs text-muted-foreground font-medium leading-snug">Percentage of available platform features toggled on.</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                  <div className="w-20 h-2 bg-muted rounded-full overflow-hidden hidden sm:flex">
                      <div className={`h-full transition-all ${getBarColor(fPct)}`} style={{ width: typeof fPct === 'number' ? `${fPct}%` : '0%' }}></div>
                  </div>
                  <span className={`w-10 text-right font-bold text-xl tabular-nums ${getScoreColor(fPct)}`}>{fPct}%</span>
                  <ChevronDown className="w-4 h-4 text-muted-foreground transition-transform group-open:rotate-180" />
              </div>
          </summary>
          <div className="px-5 pb-5 border-t border-border pt-4 text-sm text-muted-foreground space-y-3 bg-white">
            <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-foreground">{fLen} of {fTotal}</span>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
                {project?.features?.map((f: string) => (
                    <span key={f} className="px-2.5 py-1 bg-primary/10 text-primary border border-primary/20 rounded-md text-xs font-semibold">{f}</span>
                ))}
            </div>
          </div>
      </details>
      ...
      <details ref={csatRef} className="bg-white border border-border rounded-xl shadow-sm transition-all duration-300 hover:border-primary/40 group relative">
          <summary className="p-5 cursor-pointer outline-none hover:bg-slate-50 flex justify-between items-center list-none [&::-webkit-details-marker]:hidden">
              <div className="flex flex-col pr-4">
                  <span className="mb-1 text-sm font-bold text-foreground tracking-tight">Project CSAT</span>
                  <span className="text-xs text-muted-foreground font-medium leading-snug">Client satisfaction score for this specific rollout.</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                  <div className="w-20 h-2 bg-muted rounded-full overflow-hidden hidden sm:flex">
                      <div className={`h-full transition-all ${getBarColor(csatVal)}`} style={{ width: typeof csatVal === 'number' ? `${csatVal}%` : '0%' }}></div>
                  </div>
                  <span className={`w-8 text-right font-bold text-xl tabular-nums ${getScoreColor(csatVal)}`}>{csatVal}</span>
                  <ChevronDown className="w-4 h-4 text-muted-foreground transition-transform group-open:rotate-180" />
              </div>
          </summary>
          <div className="px-5 pb-5 pt-4 text-sm text-muted-foreground space-y-3 border-t border-border bg-white">
            <div className="flex justify-between items-center">
                  ...
                  <div className="relative" ref={popRef}>
                      ...
                      >
                          <span className="text-foreground font-bold truncate">{project?.csat || 'No Rating'}</span>
                      ...
                      {csatMenuOpen && (
                          <div className="absolute right-0 top-full mt-2 w-[140px] p-1 shadow-xl border border-border rounded-xl bg-white z-50">
                              <button className="w-full text-left px-3 py-2 text-sm font-semibold hover:bg-slate-100 rounded-md cursor-pointer" onClick={() => handleCsatChange('Satisfied')}>Satisfied</button>
                              <button className="w-full text-left px-3 py-2 text-sm font-semibold hover:bg-slate-100 rounded-md cursor-pointer" onClick={() => handleCsatChange('Neutral')}>Neutral</button>
                              <button className="w-full text-left px-3 py-2 text-sm font-semibold hover:bg-slate-100 rounded-md cursor-pointer" onClick={() => handleCsatChange('Dissatisfied')}>Dissatisfied</button>
                              <div className="border-t border-border mt-1 pt-1">
                                <button className="w-full text-left px-3 py-2 text-sm font-semibold text-muted-foreground hover:bg-slate-100 rounded-md cursor-pointer" onClick={() => handleCsatChange('')}>Clear Rating</button>
                              </div>
                          </div>
                      )}
                  </div>
