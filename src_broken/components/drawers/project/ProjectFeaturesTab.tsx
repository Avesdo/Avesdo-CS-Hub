       ...
       <div className="bg-white border border-border rounded-xl p-5 shadow-sm">
           <div className="flex items-center justify-between mb-3">
               <div className="flex flex-col">
                   ...
                   <p className="text-xs text-muted-foreground font-medium">{activeCount} of {totalCount} Active</p>
               </div>
               <span className={`text-2xl font-bold tabular-nums ${getScoreColor(pct)}`}>{pct}%</span>
           </div>
           <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
               <div className={`h-full transition-all duration-500 ease-out ${getBarColor(pct)}`} style={{ width: `${pct}%` }}></div>
           </div>
       </div>
            ...
            return (
              ...
              >
                <div className="flex-1 flex flex-col pr-4">
                  <span className={`text-sm font-bold tracking-tight transition-colors ${isActive ? 'text-primary' : 'text-slate-700'}`}>{feature}</span>
                </div>
