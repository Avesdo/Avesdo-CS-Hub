...
import { updateClientRecord, deleteClientRecord, updateProjectRecord, updateServiceRecord, addAutoLog, addServiceAutoLog, addProjectAutoLog } from '../../api/dbService';
...
export default function ClientDrawer() {
  ...
  const [activeTab, setActiveTab] = useState<'health' | 'trends' | 'projects' | 'services' | 'notes'>('health');
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      }, `Client Name updated from "${oldName}" to "${newName}"`, user?.name);
      ...
      if (projectsToUpdate.length > 0) {
        const projectPromises = projectsToUpdate.map((p: any) => {
           ...
           return updateProjectRecord({ ...p, clients: updatedClients }, true, `Client name updated from "${oldName}" to "${newName}"`, user?.name, true);
        });
      ...
      if (servicesToUpdate.length > 0) {
        const servicePromises = servicesToUpdate.map(async (s: any) => {
           ...
           await updateServiceRecord({ ...s, clients: updatedClients }, true, `Attached client name updated from "${oldName}" to "${newName}"`, user?.name, true);
        });
  ...
  const handleUpdateType = async (type: string) => {
    ...
    await updateClientRecord({ ...client, clientType: type }, {
    ...
    }, `Client Type changed from ${oldType} to ${type}`, user?.name);
  };
  ...
  const handleUpdateManager = async (manager: string) => {
    ...
    await updateClientRecord({ ...client, accountManager: manager }, {
    ...
    }, `Manager changed from ${oldManager} to ${manager || 'Unassigned'}`, user?.name);
  };
  ...
  const healthScore = (client?.activeProjectCount === 0 && !hasSuspendedProjects) ? 'N/A' : healthResult.totalScore;
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-border bg-slate-50 flex items-center gap-6 relative shrink-0">
            ...
            <div className="flex items-center gap-3 group mb-1">
              {isEditingName ? (
                <div className="flex items-center gap-2 w-full">
                  ...
                  />
                  <button onClick={handleSaveName} className="p-1 hover:bg-emerald-50 rounded-md text-emerald-600 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><Check className="w-5 h-5" /></button>
                  <button onClick={() => { setEditNameValue(client?.companyName || client?.name || ''); setIsEditingName(false); }} className="p-1 hover:bg-muted rounded-md text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"><X className="w-5 h-5" /></button>
                </div>
              ) : (
                <>
                  <h2 className="text-2xl font-semibold text-foreground tracking-tight leading-tight truncate">{client?.companyName || client?.name}</h2>
            ...
            <div className="flex items-center gap-3 mt-1 group relative">
               <div className="relative inline-block">
                 <Select
                    ...
                    options={(settings?.clientTypes?.map((t: any) => t.name) || []).map((t: any) => ({ label: getSettingBadge('clientTypes', t, settings), value: t }))}
               ...
               <div className="relative inline-block">
                 <Select
                    ...
                    trigger={
                      <button className="hover:-translate-y-0.5 hover:shadow-md transition-all rounded-full inline-block">
                        {getSettingBadge('managers', client?.accountManager || 'Unassigned', settings)}
                      </button>
                    }
                 />
               </div>
          ...
          <div className="absolute top-4 right-4 flex items-center gap-1">
            {isConfirmingDelete ? (
              <button onClick={async () => { if (client) { const cid = client.clientId || client.id; await deleteClientRecord(cid); await addAutoLog(cid, `Client profile archived`, user?.name || 'System'); const cProjects = projects.filter((p: any) => p.clientIds?.includes(cid)); for (const p of cProjects) await addProjectAutoLog(p.id, `Client "${client.companyName || client.name}" archived`, user?.name || 'System', true); const cServices = services.filter((s: any) => s.clientIds?.includes(cid)); for (const s of cServices) await addServiceAutoLog(s.id, `Attached Client "${client.companyName || client.name}" archived`, user?.name || 'System', true); closeDrawer(); } }} className="flex items-center gap-1.5 px-3 py-1.5 border-2 border-red-300 rounded-md text-red-600 bg-white hover:bg-red-50 font-semibold shadow-sm text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                <AlertTriangle className="w-4 h-4" /> Confirm Archive
              </button>
            ) : (
              <button onClick={() => setIsConfirmingDelete(true)} className="p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive rounded-md transition-all active:scale-95 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" title="Archive Client">
