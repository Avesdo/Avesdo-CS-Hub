...
export default function ServiceDetailsTab({ service }: ServiceDetailsTabProps) {
  ...
  const serviceOutcomes = (settings?.settingsData || []).filter((s: any) => s.category === 'ServiceOutcome');
  const serviceStatuses = (settings?.settingsData || []).filter((s: any) => s.category === 'ServiceStatus');
  ...
  const handleUpdate = async (field: string, newValue: any, oldValue: any, logPrefix?: string, forceUpdates?: any) => {
      ...
      if (field === 'outcome' && newValue === 'Won') {
          ...
          payload.dateStr = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      }
  ...
  const handleStatusChange = (newStatus: string) => {
      ...
      handleUpdate('status', newStatus, service?.status, 'Service Status', updates);
  };
  ...
  useEffect(() => { setContactDraft(service?.contactName || ''); }, [service?.contactName]);
  ...
  // Price Draft
  const [priceDraft, setPriceDraft] = useState(() => service?.price != null ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.price)) : '');
  useEffect(() => { setPriceDraft(service?.price != null ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.price)) : ''); }, [service?.price]);
  ...
  const handlePriceBlur = () => {
     ...
     if (isNaN(num)) { setPriceDraft(service?.price != null ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.price)) : ''); return; }
     handleUpdate('price', num, service?.price, 'Service Value');
     setPriceDraft(new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num));
  };
  
  // Invoice Draft
  const [invoiceDraft, setInvoiceDraft] = useState(service?.invoiceNum || '');
  useEffect(() => { setInvoiceDraft(service?.invoiceNum || ''); }, [service?.invoiceNum]);
  ...
  // Commission Draft
  const [commDraft, setCommDraft] = useState(() => service?.commission != null && !isNaN(Number(service.commission)) ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.commission)) : '');
  useEffect(() => { setCommDraft(service?.commission != null && !isNaN(Number(service.commission)) ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.commission)) : ''); }, [service?.commission]);
  
  const autoCalcCommission = () => {
     const p = Number(service?.price) || 0;
     const c = p * 0.20;
     setCommDraft(new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(c));
  ...
  const handleCommBlur = () => {
     ...
     const num = parseFloat(raw);
     if (isNaN(num)) { setCommDraft(service?.commission != null ? new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(service.commission)) : ''); return; }
     handleUpdate('commission', num, service?.commission, 'Commission Value');
     setCommDraft(new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(num));
  };
  ...
  const handleCommPaidToggle = (checked: boolean) => {
      if (checked) {
          ...
          handleUpdate('commPaid', true, service?.commPaid, 'Commission Paid', {
             ...
             commDateStr: d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
          });
      } else {
  ...
  return (
    <div className="flex flex-col space-y-8" ref={popRef}>
      ...
      {(!service?.type || !service?.status || !service?.manager) && (
          <div id="sd-error" className="bg-red-50 text-red-700 p-3 rounded-md border border-red-200 flex items-start gap-2">
             <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
             <div className="text-sm">Please ensure Service Type, Manager, and Status are populated.</div>
          </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-border items-start">
        <div id="popover-type" className="relative popover-container">
          <label className="block text-sm font-medium text-muted-foreground mb-1.5">Service Type</label>
          <div className="flex">
             <button onClick={() => setOpenPop(openPop === 'type' ? null : 'type')} className="text-left hover:-translate-y-0.5 hover:shadow-md transition-all rounded-full inline-flex">
          ...
          {openPop === 'type' && (
             <div className="absolute top-full left-0 mt-2 min-w-[200px] bg-white border border-border rounded-lg shadow-xl z-50 p-1 animate-in fade-in slide-in-from-top-2 duration-200">
               {settings?.serviceTypes?.map((t: any) => (
                 <button key={t.name} onClick={() => handleTypeChange(t.name)} className="w-full text-left px-2 py-1.5 text-sm font-medium rounded-md hover:bg-slate-50 flex items-center justify-between">
        ...
        {!isIncluded && (
          <div>
            <label className="block text-sm font-medium text-muted-foreground mb-1.5">Service Value</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-border items-start">
        <div id="popover-manager" className="relative popover-container">
          ...
          <button onClick={() => setOpenPop(openPop === 'manager' ? null : 'manager')} className="w-full flex items-center justify-between rounded-md border border-input bg-white px-3 py-2 text-sm shadow-sm transition-all duration-200 active:scale-95 hover:bg-slate-50 hover:border-primary/50 focus:outline-none min-h-[38px]">
             <span className="truncate font-semibold text-foreground">{service?.manager || 'Unassigned'}</span>
             <div className="i-lucide-chevron-down w-4 h-4 text-muted-foreground shrink-0" />
          </button>
          {openPop === 'manager' && (
             <div className="absolute top-full left-0 mt-2 min-w-[200px] bg-white border border-border rounded-lg shadow-xl z-50 p-1 animate-in fade-in slide-in-from-top-2 duration-200">
               {settings?.managers?.map((m: any) => (
                 <button key={m.name} onClick={() => { setOpenPop(null); handleUpdate('manager', m.name, service?.manager, 'Manager'); }} className="w-full text-left px-2 py-1.5 text-sm font-medium rounded-md hover:bg-slate-50 flex items-center justify-between whitespace-nowrap">
                   {m.name}
                 </button>
               ))}
             </div>
        ...
        <div>
          <label className="block text-sm font-medium text-muted-foreground mb-1.5">Client Contact Name</label>
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-border items-start">
        <div className="flex flex-col space-y-6">
            <div id="popover-outcome" className="relative popover-container">
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">Outcome</label>
              <div className="flex">
                 <button onClick={() => setOpenPop(openPop === 'outcome' ? null : 'outcome')} className="text-left hover:-translate-y-0.5 hover:shadow-md transition-all rounded-full inline-flex">
              ...
              {openPop === 'outcome' && (
                 <div className="absolute top-full left-0 mt-2 min-w-[200px] bg-white border border-border rounded-lg shadow-xl z-50 p-1 animate-in fade-in slide-in-from-top-2 duration-200">
                   {serviceOutcomes.map((s: any) => (
                     <button key={s.name} onClick={() => handleOutcomeChange(s.name)} className="w-full text-left px-2 py-1.5 text-sm font-medium rounded-md hover:bg-slate-50 flex items-center justify-between">
        ...
        <div id="popover-status" className="relative popover-container">
          <label className="block text-sm font-medium text-muted-foreground mb-1.5">Service Status</label>
          <div className="flex">
      ...
      {!isIncluded && (
        <div id="sd-invoice-container" className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-border items-center">
          <div>
          ...
          <div className="flex items-center gap-6 pt-6">
             <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={service?.invoiceSent || false} onChange={e => handleUpdate('invoiceSent', e.target.checked, service?.invoiceSent, 'Invoice Sent')} className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-0 focus-visible:ring-primary/20 focus-visible:ring-2 focus:ring-offset-0 outline-none" />
                <span className="text-sm font-semibold">Invoice Sent</span>
             </label>
             <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={service?.invoicePaid || false} onChange={e => handleUpdate('invoicePaid', e.target.checked, service?.invoicePaid, 'Invoice Paid')} className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-0 focus-visible:ring-primary/20 focus-visible:ring-2 focus:ring-offset-0 outline-none" />
      ...
      {!isIncluded && (
        <div id="sd-comm-container" className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-2 items-start">
           <div>
              <div className="flex justify-between items-center mb-1.5">
                 <label className="block text-sm font-medium text-muted-foreground">Commission Value</label>
                 <button onClick={autoCalcCommission} className="text-[11px] font-bold text-primary hover:text-primary/80 transition-colors active:scale-95">Auto Calc (20%)</button>
              </div>
              <div className="relative">
                 <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground font-semibold">$</span>
                 <input
           ...
           <div className="flex flex-col space-y-3">
               <label className="block text-sm font-medium text-muted-foreground">Commission Paid?</label>
               <label className="flex items-center gap-2 cursor-pointer mb-2">
                  <input type="checkbox" checked={service?.commPaid || false} onChange={e => handleCommPaidToggle(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-primary focus:ring-0 focus-visible:ring-primary/20 focus-visible:ring-2 focus:ring-offset-0 outline-none" />
