...
const Sparkline = ({ data }: { data: number[] }) => {
    ...
    if (endVal > startVal) strokeColor = '#10b981'; // green for increase
...
interface ProjectTrackerTableProps {
  ...
  releaseDateFilter?: { start: string, end: string } | null;
  setReleaseDateFilter?: React.Dispatch<React.SetStateAction<{ start: string, end: string } | null>>;
...
export const ProjectTrackerTable: React.FC<ProjectTrackerTableProps> = ({
  ...
  const groupA = ['Actively Onboarding', 'Upcoming (> 45 Days)', 'No Due Date'].includes(activeTab);
  ...
  const allManagers = Array.from(new Set(baseProjects.map(p => p.assignee || 'Unassigned'))).sort();
  const allTimelines = Array.from(new Set(baseProjects.map(p => p.timelineStatus || 'Not Set'))).sort();
  const allPhases = Array.from(new Set(baseProjects.map(p => p.onboardingPhase || 'Not Set'))).sort();
  
  const pt_features_fallback = ['Contracts', 'Inventory', 'Pricing', 'Deposits', 'Payments', 'Allocations', 'Workflows', 'Reporting'];
  const allFeatures = Array.isArray(settings?.features) && settings?.features.length > 0 ? settings.features : pt_features_fallback;
  ...
  return (
        <table className="w-full text-left bg-white border-separate border-spacing-0">
          <thead className="sticky top-0 z-50 bg-white/90 backdrop-blur-md">
            <tr className="bg-slate-50/80 text-slate-500 text-[11px] font-bold tracking-wider h-[45px]">
              <th className="sticky left-0 z-40 bg-slate-50/90 backdrop-blur-md border-b border-border border-r-0 px-4 py-3 w-12 group/th">
                <input 
                  type="checkbox" 
              ...
              <th className="sticky left-12 z-40 bg-slate-50/90 backdrop-blur-md border-b border-border border-r-0 px-4 py-3 group/th">
                <div className="flex items-center">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('name')}>Project Name</div>
                  {setNameFilter && <ColumnFilter options={allNames} selected={nameFilter || []} onChange={setNameFilter} searchable />}
                </div>
              </th>
              <th className="border-b border-border px-4 py-3 min-w-[160px] group/th">
                <div className="flex items-center">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('clients')}>Client(s)</div>
                  {setClientFilter && <ColumnFilter options={allClients} selected={clientFilter || []} onChange={setClientFilter} searchable />}
                </div>
              </th>
              {showHealthScore && (
                <th className="border-b border-border px-4 py-3 min-w-[120px] group/th">
                  <div className="flex items-center justify-center">
                    <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('healthScore')}>Health Score</div>
                    {setHealthFilter && (
                      <ColumnFilter options={['Healthy', 'Warning', 'At Risk']} selected={healthFilter || []} onChange={setHealthFilter} />
                    )}
                  </div>
                </th>
              )}
              <th className="border-b border-border px-4 py-3 min-w-[110px] group/th">
                <div className="flex items-center">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('releaseDateVal')}>Release Date</div>
                  {setReleaseDateFilter && <DateFilter dateRange={releaseDateFilter} setDateRange={setReleaseDateFilter} />}
                </div>
              </th>
              <th className="border-b border-border px-4 py-3 min-w-[120px] group/th">
                <div className="flex items-center">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('assignee')}>Manager</div>
                  {setManagerFilter && <ColumnFilter options={allManagers} selected={managerFilter || []} onChange={setManagerFilter} />}
                </div>
              </th>
              <th className="border-b border-border px-4 py-3 min-w-[130px] group/th">
                <div className="flex items-center">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('projectStatus')}>Project Status</div>
                  {setStatusFilter && (
                    <ColumnFilter options={['Onboarding', 'Active', 'Suspended', 'Closed']} selected={statusFilter || []} onChange={setStatusFilter} />
                  )}
                </div>
              </th>
              {showTimeline && (
                  <th className="border-b border-border px-4 py-3 min-w-[130px] group/th">
                    <div className="flex items-center">
                      <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('timelineStatus')}>Schedule Status</div>
                      {setTimelineFilter && <ColumnFilter options={allTimelines} selected={timelineFilter || []} onChange={setTimelineFilter} />}
                    </div>
                  </th>
              )}
              {showPhase && (
                  <th className="border-b border-border px-4 py-3 min-w-[150px] group/th">
                    <div className="flex items-center">
                      <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap mr-2" onClick={() => onSort('onboardingPhase')}>Onboarding Phase</div>
                      {setPhaseFilter && <ColumnFilter options={allPhases} selected={phaseFilter || []} onChange={setPhaseFilter} />}
                    </div>
                  </th>
              )}
              <th className="border-b border-border px-4 py-3 w-[80px] group/th">
                <div className="flex items-center justify-end">
                  <div className="cursor-pointer hover:text-slate-800 transition-colors whitespace-nowrap" onClick={() => onSort('units')}>Live Units</div>
                </div>
              </th>
              {showChecklist && (
                <th className="border-b border-border px-4 py-3 min-w-[100px] group/th">
              ...
              {showFeatures && (
                  <th className="border-b border-border px-4 py-3 min-w-[120px] group/th">
                    <div className="flex items-center justify-center">
                      ...
                      {setFeaturesFilter && <ColumnFilter options={allFeatures} selected={featuresFilter || []} onChange={setFeaturesFilter} searchable />}
                    </div>
                  </th>
              )}
            </tr>
          ...
          <tbody className="divide-y divide-border text-sm">
            {projects.map(p => (
              <tr 
                key={p.id} 
                className={`hover:bg-slate-50 transition-colors cursor-pointer group ${selectedRows.includes(p.id) ? 'bg-primary/5' : 'bg-white'}`}
              ...
              >
                ...
                <td className="px-4 py-3 text-muted-foreground">
                  <TruncatedText 
                    text={p.clients?.join(', ') || 'No clients'} 
                ...
                {showHealthScore && (
                  <td className="px-4 py-3 cursor-pointer" onClick={(e) => { e.stopPropagation(); openDrawer('project', p.id, { targetTab: 'health' }); }}>
                    <div className="flex items-center gap-4 group/trend p-1 -m-1 rounded hover:bg-slate-100 transition-colors w-fit">
                      ...
                      {p.trendData && (
                        <div className="opacity-80 group-hover/trend:opacity-100 transition-opacity hidden xl:block cursor-pointer" onClick={(e) => { e.stopPropagation(); openDrawer('project', p.id, { targetTab: 'trends' }); }}>
                ...
                <td className="px-4 py-3 text-muted-foreground relative" onClick={stopProp}>
                  <div className="flex items-center gap-2 hover:text-primary transition-colors hover:bg-slate-100 px-2 py-1 -ml-2 rounded-md group">
                    ...
                    <span className="whitespace-nowrap">
                        {p.releaseDateVal ? new Date(p.releaseDateVal).toLocaleDateString('en-US', { month: 'short', day: 'numeric', ...(groupA ? {} : { year: 'numeric' }) }) : 'Not Set'}
                    </span>
                    <input 
                      type="date"
                      ...
                      onChange={(e) => {
                        ...
                        const dateObj = new Date(val + 'T12:00:00Z');
                    ...
                    <td className="px-4 py-3" onClick={stopProp}>
                      <div className="inline-block relative">
                        <Select
                          ...
                          options={(settings?.phases?.map((ph: any) => ph.name) || []).map((ph: any) => ({ label: getSettingBadge('phases', ph, settings), value: ph }))}
            ...
            {projects.length === 0 && (
              <tr>
                <td colSpan={12} className="px-6 py-12">
                  <EmptyState icon={CheckSquare} title="No projects found" subtitle="Try adjusting your filters or tab selection." />
                </td>
              </tr>
            )}
          </tbody>
          {footerContent && (
            <tfoot className="sticky bottom-0 z-40 bg-slate-50">
              <tr>
                <td colSpan={100} className="px-[128px] py-[12px] h-[45px] border-t border-border shadow-[0_-1px_2px_rgba(0,0,0,0.02)] rounded-b-xl">
                  {footerContent}
                </td>
              </tr>
            </tfoot>
