...
import { Upload, ChevronRight, CheckCircle2, ArrowRight, Play, Server, FileText } from 'lucide-react';
...
export default function NotionMigration() {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Papa.parse(file, {
            ...
            complete: (results) => {
                ...
                const cMap: any = {};
                Array.from(cSet).filter(Boolean).forEach(c => {
    ...
    const generatePayloads = () => {
        ...
        csvData.forEach((row, i) => {
            ...
            const rawClients = row['Developer/Sales'] ? row['Developer/Sales'].split(',').map((c: string) => c.trim()) : [];
        ...
        <div className="max-w-4xl mx-auto animate-in fade-in duration-300 pb-12">
            ...
            <div className="flex items-center gap-2 mb-8 bg-slate-50 p-3 rounded-lg border border-border">
                {[1, 2, 3, 4].map((s) => (
                    <React.Fragment key={s}>
                        ...
                        {s < 4 && <div className={`flex-1 h-1 ${step > s ? 'bg-indigo-600' : 'bg-slate-200'}`} />}
                    </React.Fragment>
                ))}
            </div>
            ...
            {step === 1 && (
                <div className="bg-white border rounded-xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
                    ...
                    <p className="text-sm text-slate-500 mt-1 mb-6 max-w-md">Drag and drop the `notion_export.csv` file here, or click to browse. The file will be parsed locally in your browser.</p>
                ...
                <div className="space-y-6">
                    <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
                        <div className="bg-slate-50 px-5 py-3 border-b border-border">
                            ...
                            <p className="text-xs text-slate-500 font-normal mt-0.5">Map unique CSV values to official Hub features. This maps ALL instances across your 700+ projects at once.</p>
                        </div>
                        <div className="p-5">
                ...
                <div className="space-y-6">
                    <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
                        <div className="bg-slate-50 px-5 py-3 border-b border-border">
                            ...
                            <p className="text-xs text-slate-500 font-normal mt-0.5">Define whether each extracted client is a Developer or Sales & Marketing entity. You can also correct typos (e.g. rename "Avesdo" to "Avesdo Tech"), and it will apply globally to all projects assigned to that client.</p>
                        </div>
            ...
            {step === 4 && (
                <div className="space-y-6">
                    ...
                    <div className="flex justify-between pt-4">
                        <button onClick={() => setStep(3)} className="flex items-center gap-2 bg-white border border-slate-300 text-slate-700 px-6 py-2.5 rounded-md font-semibold hover:bg-slate-50 transition-colors shadow-sm" disabled={isExecuting}>
                            Back to Mapping
                        </button>
                        <button 
