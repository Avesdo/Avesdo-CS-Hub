...
export const universalExportCSV = (entityType: 'Clients' | 'Projects' | 'Services', data: any[], filenamePrefix: string) => {
    ...
    if (entityType === 'Clients') {
        ...
        headers = ['Project Name', 'Client(s)', 'Health Score', 'Release Date', 'Manager', 'Project Status', 'Schedule Status', 'Onboarding Phase', 'Live Units', 'Active Features', 'Deliverables Checklist'];
        csvRows = data.map(mapProjectRow);
    } else if (entityType === 'Services') {
        headers = ['Service Name', 'Project Name', 'Service Type', 'Client Name', 'Manager', 'Service Value', 'Outcome', 'Service Status', 'Date', 'Invoice Sent', 'Invoice Paid', 'Commission Value', 'Commission Paid', 'Date Paid'];
