...
const validLines = lines.filter(l => !l.includes("null_prod") && !l.startsWith("Event Title") && l.trim().length > 0);
console.log("Sessions Started valid lines sample:");
validLines.slice(0, 5).forEach((l, i) => console.log(`Row ${i}:`, l.substring(0, 200)));
console.log("Total valid rows in Sessions Started:", validLines.length);
...
const validLines2 = lines2.filter(l => !l.includes("null_prod") && !l.startsWith("Event Title") && l.trim().length > 0);
console.log("\nPage Views valid lines sample:");
validLines2.slice(0, 5).forEach((l, i) => console.log(`Row ${i}:`, l.substring(0, 200)));
console.log("Total valid rows in Page Views:", validLines2.length);
