...
- Name: phase2.spec.ts >> Phase 2: The Dashboard >> Dash Drilldown Drawer
- Location: tests\phase2.spec.ts:18:3
...
Locator: locator('div').filter({ hasText: /^Launch Pipeline/ }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('div').filter({ hasText: /^Launch Pipeline/ }).first()
