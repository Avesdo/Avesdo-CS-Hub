...
import { ClipboardList, CheckCircle2, ShieldCheck, FileText, ChevronRight, Award, ChevronLeft, ExternalLink } from 'lucide-react';
...
export default function ClientPortal() {
  ...
  const handleSaveForm = async (data: any, autoSave = false, overrideStatus?: string) => {
    ...
    if (isSaveProgress) {
      toastId = toast.loading('Saving progress...');
    }
    ...
    try {
      ...
      if (!autoSave && !overrideStatus && isFirstSubmission) {
      ...
      const prettyFormName = activeFormType === 'onboardingCsat' 
        ? 'Onboarding CSAT' 
        : activeFormType === 'survey' 
          ? 'Onboarding Survey' 
          : activeFormType.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase());
        ...
        <div className="max-w-3xl mx-auto w-full">
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-500">
            <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-10 text-center text-white">
              ...
              <p className="text-emerald-50 max-w-xl mx-auto text-lg opacity-90">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          {forms.map(form => {
            ...
            if (form.id === 'deliverables') {
              ...
              isCompleted = !!dataNode?.submittedAt;
            }
            ...
            return (
              ...
              >
                ...
                <div className="shrink-0 flex items-center gap-4 ml-2">
                  {isCompleted ? (
                    <span className="text-xs font-bold px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-100 shadow-sm">
                      Completed
                    </span>
                  ) : (
