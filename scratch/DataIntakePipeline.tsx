                  ...
                  <div className="flex gap-3">
                    <div className="flex-1 bg-white rounded-lg shadow-sm">
                      <SearchableSelect
                        options={
                          alias.type === 'client'
                            ? clients.map((c) => ({ id: c.clientId || c.id, label: c.companyName || 'Unnamed Client' }))
                            : alias.type === 'project'
                              ? projects.map((p) => ({ id: p.id, label: p.name || 'Unnamed Project' }))
                              : alias.type === 'service'
                                ? services.map((s) => ({ id: s.id, label: s.name || 'Unnamed Service' }))
                                : []
                        }
