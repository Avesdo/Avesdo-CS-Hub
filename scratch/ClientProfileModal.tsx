...
export default function ClientProfileModal() {
  ...
  const toggleProjectArrayItem = async (projectId: string, projectName: string) => {
    ...
    await updateProjectRecord(
      ...
      {
        successMsg: `Projects successfully updated`,
        errorMsg: `Failed to update Projects`,
      },
      logMsg,
      user?.name
    );
  ...
  const toggleServiceArrayItem = async (serviceId: string, serviceName: string) => {
    if (!client) return;
    const targetService = services.find(s => s.id === serviceId);
    if (!targetService) return;

    const cIds = targetService.clientIds || [];
    const cNames = targetService.clients || [];
    const isRemoving = cIds.includes(client.clientId || client.id);
    ...
    const logMsg = isRemoving
      ? `Client "${client.companyName || client.name}" detached from service ${serviceName}`
      : `Client "${client.companyName || client.name}" attached to service ${serviceName}`;
    ...
    await updateServiceRecord(
      ...
      },
      {
        ...
        errorMsg: `Failed to update Services`,
      },
      logMsg,
      user?.name
    );
  ...
  const filteredServices = useMemo(() => {
    ...
    if (!serviceSearch) return sorted;
                ...
                <div className="w-[320px] bg-slate-50/80 border-r border-slate-200/60 flex flex-col shrink-0">
                  ...
                  <div className="flex-1 overflow-y-auto custom-thin-scroll px-6 pb-6 flex flex-col gap-5">
                    ...
                    <div className="flex flex-col gap-6 pt-6 border-t border-slate-200/60">
                      {/* Attached Projects */}
                      <div className="flex flex-col gap-2 group/projects">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[11px] font-bold text-slate-500 tracking-wider">Attached Projects</span>
                          <Popover.Root open={popoverOpen === 'projects'} onOpenChange={(o) => setPopoverOpen(o ? 'projects' : null)}>
                            <Popover.Trigger asChild>
                              <button className="opacity-0 group-hover/projects:opacity-100 flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 hover:text-slate-700 transition-all">
                            ...
                            <Popover.Content side="right" sideOffset={12} className="z-[110] w-[300px] bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden flex flex-col max-h-[300px]">
                              ...
                              <div className="overflow-y-auto p-1 custom-thin-scroll">
                                {filteredProjects.map((p) => {
                                  const isSelected = p.clientIds?.includes(client?.clientId || client?.id);
                                  return (
                                    <button
                                      key={p.id}
                                      onClick={() => toggleProjectArrayItem(p.id, p.name)}
                          ...
                          if (attachedProjects.length > 0) {
                            return (
                              <div className="flex flex-col gap-1.5">
                                {attachedProjects.map((p: any) => (
                                  <div key={p.id} className="flex items-center gap-2 text-[13px] font-medium text-slate-700 bg-white border border-slate-200/60 px-3 py-2 rounded-xl shadow-sm">
                            ...
                            return <span className="text-[12px] italic text-slate-400 px-1">None attached</span>;
                          }
                        })()}
                      </div>
                    </div>
